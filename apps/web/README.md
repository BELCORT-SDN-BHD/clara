...
| Settings | `/settings`, `/settings/account`, `/settings/firm`, `/settings/members`, `/settings/setup`, `/settings/compliance`, `/settings/vendor-bindings` | The caller's own account, plus capability-shaped firm administration. `/settings/setup` (admin+; #648) is journey A5's resumable list of the facts the firm still owes about itself, read from `clara.get_firm_setup()` and written through the four `clara.*_firm_setup*` doors; it gates nothing else in the workspace |
