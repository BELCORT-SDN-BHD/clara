          ...
          <ul className="flex flex-col gap-1 text-sm">
            {(templates.data ?? []).map((tpl) => (
              <li key={tpl.id} className="flex flex-wrap gap-3 rounded-md border border-border p-2">
                ...
                <span className="text-muted-foreground">{t("status")}: {tpl.status}</span>
                <span className="text-muted-foreground">{t("cadence")}: {tpl.cadence}</span>
              </li>
            ))}
          </ul>
        </DataState>
              ...
              <tbody>
                {(runs.data ?? []).map((r) => (
                  <tr key={r.id} className="border-b border-border last:border-0">
                    ...
                    <td className="py-2 pr-4 text-card-foreground">{r.mode}</td>
                    <td className="py-2 pr-4 text-card-foreground">{fmtCents(r.amount_cents)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
