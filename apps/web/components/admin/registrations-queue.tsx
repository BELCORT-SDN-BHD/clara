...
function OperatorQueue() {
  ...
  async function handleApprove(row: RegistrationRequestRow) {
    ...
    setReceipt(null);
    await act(async () => {
      const out = await approveFirmRegistration(sessionTokenAccessor, row.id);
  ...
  async function handleReject(row: RegistrationRequestRow, reason: string, onOk: () => void) {
    setReceipt(null);
    await act(async () => {
      await rejectFirmRegistration(sessionTokenAccessor, row.id, reason);
    }, onOk);
  }
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {!rows ? (
        <LoadingState>{t("queueLoading")}</LoadingState>
      ) : rows.length === 0 ? (
...
function RegistrationRow({
  ...
  onReject: (reason: string, onOk: () => void) => Promise<void>;
}) {
  const t = useTranslations("Registrations");
  return (
    <TableRow>
      ...
      <TableCell className="max-w-xs text-muted-foreground">{row.note ?? "—"}</TableCell>
      ...
      <DialogContent>
        ...
        <DialogFooter>
          ...
          <Button
            ...
            onClick={async () => {
              const ran = await runOnce(guardRef.current, () => onReject(reason.trim(), () => setReason("")));
