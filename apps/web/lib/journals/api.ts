 ...
 *  `client_id` is filtered here, not left to the policy. */
export async function listJournalEntries(
  session: SessionTokenAccessor,
  clientId: string,
  signal?: AbortSignal,
): Promise<JournalEntryRow[]> {
  return getRows<JournalEntryRow>("journal_entries", {
    select: ENTRY_SELECT,
 ...
 *  — packages/db/migrations/0011_daily_loop.sql:3748, current body
 *  0016_a21_compliance_watch.sql:4558 ("create or replace"; no later
 *  redefinition exists at frontier 0136). Granted to clara_authenticated at
 *  viewer+ (`_human_ctx(role_rank('viewer'))`, 0016:4563) — a plain read, not
 ...
 *  stays `null` here — this tab does not paginate past `p_limit` (a known
 *  limitation for a client with more than `p_limit` pending queue rows across
 *  EVERY row_kind combined, reported, not worked around).
 *
 *  Returns the union of draft/uncoded_filing/open_question/coding_task/
 *  compliance_watch rows city-wide for the scope; this tab keeps only
 *  `row_kind === 'draft'` (the journal-entry rows) — every other kind belongs to
 *  a different, not-yet-built tab's own queue. `row_kind`/`section` are rendered
 *  VERBATIM by the caller — this mapper never relabels or reinterprets either. */
export async function listReviewQueue(
  ...
  return arr(envelope.rows)
    .map(toReviewQueueRow)
    .filter((row) => row.row_kind === "draft");
}
...
export async function loadJournalsWorkbench(
  ...
  const [entries, lines, accounts, counterparties, queueRows] = await Promise.all([
  ...
  return { entries, lines, accounts, counterparties, queueRows };
}
 ...
 *  default null, p_op_key text default null) — packages/db/migrations/
 *  0004_governed_fns.sql:519, current body 0015_ar_myinvois_rules.sql:1536-1545
 *  (no later redefinition at frontier 0136). Bookkeeper+ (`_human_ctx`). The
 *  revision/idempotency pattern this ports from apps/dashboard/app/chat/
 ...
 *  token" (0016_a21_compliance_watch.sql:4814-4816, `_approve_entry_core`); a
 *  fresh `p_op_key` per call makes a retried click idempotent
 *  (`_reserve_op`/`_finish_op`, same file). A high-stakes entry needing a
 *  DISTINCT checker from its maker raises CLR05 — rendered verbatim, never
 *  retried (doors.ts's header). */
 ...
 *  jsonb default null, p_duplicate_override jsonb default null) —
 *  packages/db/migrations/0009_coding_floor.sql:1750, current body
 *  0016_a21_compliance_watch.sql:4765-4909 (no later redefinition at frontier
 *  0136). For a manual entry (`coding_kind is null`) neither a counterparty
 *  proposal nor cited evidence is required (0016:4817-4827 gates both on
 *  `coding_kind in ('supplier_bill','sales_invoice','sales_credit_note')` only)
 *  — this workbench always passes `null` for both, matching the manual-compose
 *  ceremony `draftManualEntry` below uses. Replaces EVERY line (DELETE + bulk
 *  INSERT keyed by array position, 0016:4835-4840) — the caller sends the full
 *  new line set, not a delta. Returns `{revision_token}`: the NEW token,
 *  required for the next `approveEntry` call on this same entry (hydrate-never-
 *  trust: the caller still re-reads the row rather than trusting this value as
 *  the entry's whole new state). CLR06 stale revision token; CLR07 unbalanced
 *  by more than 5c; CLR22 only a draft can be revised. */
 ...
 *  correction door. packages/db/migrations/0004_governed_fns.sql:560, current
 *  body 0009_coding_floor.sql:1697-1748 (no later redefinition at frontier
 *  0136). Bookkeeper+. Refuses CLR10 unless the target is `status='approved'`,
 *  not itself a reversal, and not already reversed; requires a non-blank
 *  `p_reason`. Creates a mirror entry with every line's debit/credit SWAPPED
 *  (0009:1721-1724) and auto-approves it UNLESS it is itself high-stakes, in
 *  which case the mirror lands as a new DRAFT needing its own separate approval
 *  (0009:1726-1734) — the receipt's `status` reports which happened, but this
 *  module treats that as a REPORT only (doors.ts's header): the caller re-reads
 *  the entry list rather than painting the receipt's status as new UI truth. */
...
//      never redefined since (grepped: one definition, one call site). Stamps
//      `method='human'` regardless of `p_method` (0004:489,496). This module
//      always sends `p_subject_kind: 'manual'`, `p_subject: null`,
//      `p_confidence: 1` — `assert_client_resolved`'s current body
...
//      null) — packages/db/migrations/0004_governed_fns.sql:499, current body
//      0009_coding_floor.sql:1414-1430 (granted signature confirmed at
//      0009:1427-1430; no later redefinition at frontier 0136). Bookkeeper+.
//      `_draft_entry_core` (0016_a21_compliance_watch.sql:3970 is its current
//      body) requires >=2 lines, every account_code to resolve to an ACTIVE
//      account, and balances to within 5c (auto-rounds 1-5c onto the client's
