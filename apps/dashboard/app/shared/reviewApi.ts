...
import { toSweepRun, toOpenQuestion, toCodingLane, type SweepRun, type OpenQuestion, type CodingLane } from "./reviewCardTypes";
// toRulePostRun/toAutopostRule/toNotification (autopost-rule reads) and
// toSalesEvidencePreview/../rules/model (the OCR sales-evidence preview) RETIRED with
// F-A2 PR-3, along with every DB verb they hydrated (Annex B.1) and the /rules page
// that was their only caller.
...
// Wave-A2 posted-by-rule + autopost-rule reads (getRulePostRun, listAutopostRules,
// previewOcrSalesEvidence, listRuleNotifications) RETIRED with F-A2 PR-3 — every DB
// verb they hydrated is on Annex B.1's RETIRE list.
...
// signCodingRule / declineCodingRule RETIRED with F-A2 PR-3 (Annex B.1).
...
// acknowledgeRulePosts (Wave-A2 governed writer) RETIRED with F-A2 PR-3 (Annex B.1).
...
// The typed rule-write result (RuleWriteResult/narrowRuleWrite/ruleWriteRefusedError)
// and signAutopostRule/retireAutopostRule/proposeAutopostRule RETIRED with F-A2 PR-3 —
// every autopost-rule verb they wrapped is on Annex B.1's RETIRE list, and no other
// caller in this file used the typed-refusal narrowing (checked: vendor identity
// binding below throws CLR36 directly instead).
