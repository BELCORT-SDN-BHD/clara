...
// Node built-ins only.
//
// REGISTRATION. `.claude/settings.json` (and `.claude/hooks/`) are gitignored in this repo
// (.gitignore: `.claude/*` then `!.claude/skills/` — only skills/ is tracked), so there is
// nothing under .claude/ to merge THIS hook into yet in this worktree, and none of the places a
// prior registration could live (this worktree, the main checkout's .claude/settings.local.json,
// the user-level ~/.claude/settings.json, every sibling refactor worktree) had one at the time
// this was written — so this ships as the FIRST tracked registration, living under scripts/
// (an explicitly sanctioned home per the dispatch brief) rather than under the gitignored
// .claude/hooks/. To wire it into a project or global settings.json, merge (never overwrite)
// this into hooks.PreToolUse:
//
//   {
//     "hooks": {
//       "PreToolUse": [
