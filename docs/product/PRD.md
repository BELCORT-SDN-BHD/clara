...
- **The OPERATIVE capability matrix is 裁-187's** *(ADR-0078, 2026-09-04)* — the same four ranks, with the floors moved and **RBAC as the only human gate**: **viewer** reads · **bookkeeper** uploads, drafts, matches, answers Clara **and approves and posts ANY amount, own drafts included** · **admin+** additionally begins, finalizes and abandons a close, approves the opening seed and holds firm settings · **owner** alone holds members, legal signatures (DPA, terms, the 裁-186 declaration) and the operator-tier acts. Nothing new is minted; every attestation ceremony and maker-checker wall below is abolished, and each governed click writes an automatic receipt instead.

One-firm-per-user; the last active owner cannot be removed. **A subordinate never acts on a
strict superior** (equal rank stays allowed: owner-on-owner, admin-on-admin), and **no
...
- ~~On the **high-stakes lane** (tax-affecting, closed-period, large-amount, year-end close, opening balances) the checker **must be a different human** from the last human editor — a hard DB gate where the firm has ≥2 eligible staff.~~ **SUPERSEDED by ADR-0078 (裁-187, 2026-09-04):** the distinct-checker gate, the high-stakes threshold and the solo-firm attestation are ABOLISHED; maker and checker identities stay recorded on every entry; the gate is RBAC — bookkeeper approves and posts any amount (own drafts included), close is admin+, members/legal are owner-only.
...
- ~~**Solo firms** (one eligible human) record an explicit self-approval attestation instead of being hard-blocked.~~ **SUPERSEDED by ADR-0078 (裁-187, 2026-09-04):** the solo-firm self-approval attestation is abolished with every other attestation ceremony — a solo firm's bookkeeper+ simply approves, and the automatic receipt records that maker and checker were the same identity.
...
3. **Live agent chat** — firm-altitude + per-client context, rendering Clara's clarify / plan / choice / analysis / document / tool-result / approval cards as a real generative-UI card system (fixes D-2..D-7). *(This enumeration is a promise about reply SHAPE, not registry key names — the live catalog's 26 typed part kinds cover it; e.g. "approval" rides `je_review`.)*
...
12. **Tax — draft computation** (last v1 slice; may slip to v1.1 — **RESOLVED 2026-08-24, see §9.4: owner ruled F-T3 ALL-IN for Wave F, not slipped**; then **TRUED 2026-08-31, 裁-62/裁-80: the tax module is INERT AT BETA** — Track B paused, F-T3 PR-7 held, no tax artifacts issue, the Tax tab an honest shell; v1 scope itself unchanged) — add-back engine, capital-allowance schedule, chargeable income, Form C/P/B + CP204 estimate. Clara prepares; the human reviews and e-files. **Clara never submits to LHDN.**
...
15. **Reporting & exports — TWO TIERS by law (digest law 74).** The **formal tier**: standard exports (TB, journals, GL, MA/FS, AR/AP aging, SST-02) **and** flexible reports, produced through the open→evaluate→seal→render chain, all schema-driven with authoritative DB numbers; every export persisted as an auditable artifact with parameters, data version, permissions, reproducibility (fixes H-1/H-2/H-4). The **analysis sandbox tier**: free exploration whose outputs are watermarked non-authoritative and structurally unreachable by the seal chain. *(R2 wording, owner-approved word by word 2026-08-29 — `docs/plan/active/r2-prd-two-tier-wording-draft-2026-08-27.md`.)*
...
9. ~~**Maker/checker is modelled and enforced on the high-stakes lane** (§2) for the HUMAN lane; the agentic lane routes to no human checker at any amount, and "the agent never satisfies a human sign-off" narrows to the §2 enumerated human acts *(scoped by ADR-0071)*.~~ **SUPERSEDED by ADR-0078 (裁-187): maker/checker is RECORDED, never enforced; RBAC is the human gate.**
...
16. **Client data egress is governed** *(split 16(a)/(b) by 裁-114, 2026-09-02 — the truing that names the shipped mechanism)*. **16(a) Document/OCR egress:** a client document leaves Clara only under the **typed, purpose-scoped consent subsystem** (`0020_typed_consent.sql`, with `0011_daily_loop.sql`'s client consents and `0123_f_a7_gamma_egress.sql`'s firm-level half; ADR-0040/0041; digest law 58) — client- and firm-level purpose consents PLUS a separate owner **activation**, re-checked at the **dispatch boundary** (`egress_dispatch_authorizations` and the activate/deactivate/consume doors): a grant alone never authorizes. The beta DPA (`docs/ops/legal/clara-beta-dpa.md`) rests on this gate. **16(b) Trace export** (Gate-1 C6, as ratified by **ADR-011** (`docs/adr/`) which governs on any conflict) — run traces are written to **Clara-controlled storage** (our own Postgres); **cloud-vendor trace export ships OFF** and may be enabled later only **minimized-first** and only after an executed **DPA + firm-facing MIA client authorization** (a DPA regulates the processor but does not by itself confer the authority to disclose client information outside the firm) + a documented PDPA cross-border basis + short retention + tested deletion; no vendor training on firm data. *(**16(a):** the per-client evidence rung relaxed to the firm-level DPA-stage declaration by ADR-0078 / 裁-186, 2026-09-04 — one declaration signed with the DPA is admissible evidence for the client-egress grant, and the typed per-purpose activation and the dispatch-boundary re-check are untouched. **16(b)'s trace-export gate is NOT relaxed**: the firm-facing MIA client authorization it requires is unchanged)*

**Split-trust corollary** *(re-stated by 裁-114, 2026-09-02, to the wall that actually holds; example corrected by 裁-142, 2026-09-03)*: the browser holds only its session JWT; forced RLS + EXECUTE-only grants are the isolation boundary. Reads go via RLS-scoped selects/read fns; writes go only via audited SECURITY-DEFINER functions. **No service credential ever reaches a browser, and no `NEXT_PUBLIC_`-prefixed variable ever carries one**; the agent service holds the standing service credentials, and `apps/web`'s **server-only Route Handlers** are a second, browser-isolated holder where a flow requires it (the invite mailer's `service_role` use; FS-4's `STRIPE_SECRET_KEY` for the checkout route — the Stripe webhook signing secret is RUNTIME env, never apps/web's).
...
- **Layer 2 — the typed authority.** A thin typed layer holds the posting gates: user-gated account-mapping rules, the DB-enforced ≥0.95 client gate (**AMENDED by digest law 79** — the gate now also admits the agent's own `method='judgement'` resolution under structural walls; see §6 invariant 2(a)), and first-class **open-question (must-ask) objects** with resolution state that block workflows until answered. Memory-note needs map here: observation/profile → typed client-profile facts with provenance; must_ask → open-question objects; rule_hint → low-evidence rule proposals.
...
| A full draft tax computation as a hard v1 gate | It is the last v1 slice; may slip to v1.1 (Gate-1 C5). **RESOLVED 2026-08-24, see §9.4: owner ruled F-T3 ALL-IN for Wave F, not slipped.** **TRUED 2026-08-31 (裁-62/裁-80): INERT AT BETA — built scope unchanged, nothing issues until tax's own sitting.** |
