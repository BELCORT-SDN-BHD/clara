...
3. **Billing model + scale guardrails** — split: **the per-firm token/usage-cap DESIGN half is RESOLVED** by digest laws 76/81 (meter, never cap — no budget ever pauses automation; ONE ledger, no brakes, one architecture); **the billing/pricing model remains OPEN** — the pre-public-launch gate. §8's interim guardrail (email-verify + fail-closed admission + per-firm metering/budgets/concurrency) is what stands until pricing is answered.
