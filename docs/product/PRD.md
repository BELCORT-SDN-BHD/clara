...
12. **Tax — draft computation** (last v1 slice; may slip to v1.1 — **RESOLVED 2026-08-24, see §9.4: owner ruled F-T3 ALL-IN for Wave F, not slipped**) — add-back engine, capital-allowance schedule, chargeable income, Form C/P/B + CP204 estimate. Clara prepares; the human reviews and e-files. **Clara never submits to LHDN.**
...
| A full draft tax computation as a hard v1 gate | It is the last v1 slice; may slip to v1.1 (Gate-1 C5). **RESOLVED 2026-08-24, see §9.4: owner ruled F-T3 ALL-IN for Wave F, not slipped.** |
...
3. **Billing model + scale guardrails** — the pre-public-launch gate; per-firm token/usage cap design. **PART-RESOLVED 2026-08-26: the pricing SHAPE is ruled (base monthly tier per firm + metered overage, owner; record: `harness-audit-rulings-2026-08-26.md` R8c) — amounts remain OPEN for the pricing sitting; the usage-cap half was already settled by laws 76/81.** §8's interim guardrail (email-verify + fail-closed admission + per-firm metering/budgets/concurrency) is what stands until pricing is answered.
4. **Tax-computation v1 vs v1.1 — RESOLVED.** The owner ruled F-T3 ALL-IN for Wave F, not slipped to v1.1; see `docs/plan/active/tax-computation-design.md` v1.2 and `PROGRESS.md`'s F-T3 lane. Kept here for provenance; no longer open.
