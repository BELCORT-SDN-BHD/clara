...
**Split-trust corollary** *(re-stated by 裁-114, 2026-09-02, to the wall that actually holds)*: the browser holds only its session JWT; forced RLS + EXECUTE-only grants are the isolation boundary. Reads go via RLS-scoped selects/read fns; writes go only via audited SECURITY-DEFINER functions. **No service credential ever reaches a browser, and no `NEXT_PUBLIC_`-prefixed variable ever carries one**; the agent service holds the standing service credentials, and `apps/web`'s **server-only Route Handlers** are a second, browser-isolated holder where a flow requires it (the invite mailer's `service_role` use; FS-4's Stripe webhook signing secret).
