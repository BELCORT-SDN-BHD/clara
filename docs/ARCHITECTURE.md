...
SERVING until the next runtime deploy** — this is the repo's source of record; the SERVING Fly
bundle rides `v72`, not the already-queued `v71` (`packages/runtime/README.md`'s own "the
registry pin is not the serving bundle" law). The separate,
later BANK-rules machine retired WHOLE at `0129` (F-A3 PR-3, 2026-08-26 ceremony) the same way.
