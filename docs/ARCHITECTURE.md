...
这张图描述目标统一入口。当前 registry 选择 chatTurn v19、claraWork v2、autoDraft、facts、bank、close 等流程。
...
以 AST 结构强制这一形状）：`workflowBodies`（这个镜像**能运行**的全部 body 标识符，当前 49 个，恰好等于
...
所以 `clara.document_regions` 也带一条子表侧的 belt——已记录裁决会被这条区域改写时，直接 CLR10 拒绝这次写入
（而不是再记一条相互矛盾的裁决，因为这张表是 append-only）；它只为构成恒等式的七个 `field_path` 排队，
因此五万格的 XLSX 摄取不会把五万个条目压进延迟触发器队列。月结单那一半由 0038 自己挂在
`clara.bank_statement_lines` 上的 belt 保证（line_count 一致性），0191 的 tail 断言它仍在。
