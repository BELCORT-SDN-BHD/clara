...
84. **The paid-beta gate** — three walls + Stripe checkout success; RM0/"trial" until the pricing sitting; nothing invoices until `amounts_ruled`. *(裁-57 · 58 · 68 · 73)* **85. Honest notes for a paused lane are a lawful permanent state**, swept against the lane's `PROGRESS.md` row. *(the 2026-08-31 direction)* **Law 28 KEPT** — the Codex-build/opus-review split is the cross-model pass; a native-built money surface adds a Codex read-only leg *(裁-84 → 裁-86)* — **the read-only leg then TIME-BOXED SUSPENDED until beta live by 裁-111 (owner, 2026-09-01; see law 28's own note)**; **every frontend train walks its journey in a real browser on the built app** *(裁-86)* **— AMENDED by 裁-192 (owner, 2026-09-04; permanent as a CI shape until the owner reverses it): the smoke walk on the built app BECOMES a required per-PR CI job under the `ci` meta-gate — gate AND acceptance, no longer acceptance only — once the two known `checkout-gate-walk.spec.ts` flakes are fixed at their cause; the job is not built yet. See §15 row 99.**; **law 83's ceremony is post-beta** *(裁-76)*. **The Codex BUILD lane is suspended too, by 裁-133 — see law 28 and §15.**

## 15 · Sprint-ruling amendments (裁-140, 2026-09-02; each ruling's TEXT stays in its ledger, which governs on any divergence) — **the rows live in [`README-rulings-2026-09.md`](README-rulings-2026-09.md); the law below stays here**
86. **A sprint ruling enters the ADR system as a DIGEST ROW plus an "amended by" line on the ADR it amends — never as a new ADR** *(裁-140, owner, 2026-09-02 ~23:40; permanent)*. One row, one dated line in [`README-log.md`](README-log.md), each stating its TIME BOX and pointing at the ledger entry — whose text governs on any divergence; a new ADR is minted only when a ruling contradicts an ADR's text outright AND permanently. Rationale: the sprint's rulings are mostly time-boxed, so writing each as a permanent ADR would manufacture the next stale record; a consolidating ADR was offered and declined. **THE ROWS THEMSELVES NOW LIVE IN [`README-rulings-2026-09.md`](README-rulings-2026-09.md)** — the rest of this item and item 87 (裁-111 · 114 · 125 · 129 · 131 · 133 · 135 · 139 · 141 · 143 · 144 · 145 · 146) were moved there BYTE-FOR-BYTE on 2026-09-03 (md5 `e860d0fc8186a233722af7385ff2dae0`, 16,777 bytes, proven on both sides of the move), and every row minted after it — 裁-147 · 148 · 149 · 150, then **151…185 (the launch night) as items 92–97** — is written there and never here. The reason is mechanical: this file stood at exactly 500 lines, and the ceiling is the PreToolUse max-file-size hook rather than a CI gate, so the next row would have BLOCKED an agent's write instead of reding a run. The split follows the ledgers' own convention and the plan index's path-stability rule; the ledgers stay the source of truth (`docs/plan/active/mohe-grill-rulings-2026-09-02.md` for 裁-114/125/129/131, `-09-02-pm.md` for 裁-132…145, `-09-03.md` for 裁-146 onward, and `-09-01-pm.md` for 裁-111).
