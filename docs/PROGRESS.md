...
- Updated: 2026-09-10 (MYT), after the second `/implement` session closed #614 (scoped shell, client switching, legacy-route migration) and its hosted follow-up (PR #700, malformed client id).
- `main` at 3f4c5f8b (PR #697 a916bc91+e967b442, docs PR #699 f3639671, fix PR #700 3f4c5f8b — each fast-forwarded after green CI) plus this state update. `main` requires the `ci` check, so every push goes branch → PR → green → fast-forward of the same SHA.
- Hosted: `clara-web` version `5dcee6d8-ef25-4f0e-bb9c-c2d4dd8554a0` (tag `refresh-3f4c5f8b`) promoted at 100% on Cloudflare Workers from commit 3f4c5f8b on 2026-09-10 11:01 MYT (Linux build in a clean clone, uploaded from that same tree — the bundle records the build tree's absolute paths — only the two public Supabase values + `CLARA_BUILD_SHA` at build time; all six secret names attached). Previous versions: `65d6e906` (e967b442, the #614 shell) and `90c1a5d0` (pre-refresh) are the rollback points. `clara-runtime` **v77** (image `refresh-6d4efd3d`, Node 22) and migration **0177** unchanged since the first session; seven lane DSNs `sslmode=verify-full`.
- Tickets: #606, #616, #617, #618 closed in the first session; **#614 closed** on 2026-09-10 with local + hosted evidence on the ticket (unit 2817/2817, browser 138–139 passed / 7 live-stack skips; hosted redirect matrix and login smoke; the owner signed in and the agent walked the shell in the desktop-app pane: switcher, `/work?view=needs-you`, old `/admin` bookmarks, 640px sheet, ⌘K, unknown and malformed client ids). The walk found one defect — a malformed client id hit PostgREST (400 `22P02`) and rendered the error boundary — fixed and re-verified hosted via PR #700.
