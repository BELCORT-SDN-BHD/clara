...
一批 100 份文件中，5 份缺资料，不应阻止其余独立的 95 份继续。用户能看到各自结果和等待原因；跨整批的聚合进度（如“95/100 已完成”）尚未交付，待 #636 的批次生产者。换人接手、问题过期或刷新页面不会让工作消失；继续执行仍须有当前有效的权限（chat 车道的 hook 失联恢复仍有已知缺口，待 #764）。
...
每条知识都带一个**类别**——用户陈述的事实、来自具体来源版本的提取事实、长期偏好，或关于账务如何编制的政策——
以及一个**可信级别**：由来源决定，不由写入方声明。用户陈述与访谈答案是"已陈述"，文件提取是"来自文件"，
导入包是"导入、未核实"，模型推测是"未核实推测"。政策只接受"已陈述"：导入文件自称的 verified 或模型的自信
都不能变成政策、工具注册、入账授权或未来计划授权。更正与撤回是**可归属的修订**：记录谁改的、为什么改，
被替换的值仍可读；撤回对该记录是终局，之后重新陈述同一件事是一条新记录。Knowledge 界面按类别呈现，
标出适用条件与有效期，并把"成功但为空""筛选后无结果""来源不可读""两条在世记录相互矛盾""读取失败"
作为五种不同状态分别说明，而不是混成一个空列表。

对话现在也是这条车道的入口：用户在 chat 里说出一件关于客户的长期事实，Clara 可以直接把它记进知识登记簿——键必须是服务器持有的注册键，写入以**说话的那个人**的实时权限为准，可信级别仍由来源决定（政策类记录拒绝模型推测，哪怕提问的是事务所 owner）。反过来，Clara 在每个回合之前读一次该客户的知识；**读不到的时候它会直说读不到**，并附上原因，而不是当作"这个客户没有记录"去请用户再说一遍已经说过的事。

在客户上下文中说“以后这样处理”，默认只适用于该客户。明确要求全 firm 使用，并具备相应权限时，才形成事务所默认规则；已有客户例外继续保留（该晋升路径的数据库门已就绪，但目前没有界面或对话入口调用它，待 #648）。

已完成的工作和纠正可以积累可追溯经验，供后续参考。反复采用某种做法，不会自动把它变成已确认政策，也不会授权未来持续产生分录的计划。修正或撤回依据时，依赖它的经验也要能被重新评估（自动重新评估的消费者尚未交付，待 #658／#663；目前更正或撤回后需人工复核受影响的经验）。删除普通聊天仍保留支撑已确认知识的最少、可读依据，不保留一份隐藏的完整聊天。
...
负责人完成邮箱确认、适用条款和资料处理声明、产品准入及结账步骤，取得自己的事务所空间。邀请同事后，成员可以接受邀请、恢复账户并按职责工作。数据处理用途与告知应清楚；一次事务所声明不能变成无限制披露客户资料的许可（当前 accounting_work 用途的模型外发授权由事务所一次性 Terms/DPA 接受自动派生、覆盖全部活跃客户、无逐客户开关——这条设计待 owner 对 #825 的裁定）。
...
到期日是一个有时区的日历日：当前支持的时区是 `Asia/Kuala_Lumpur`（单一封闭取值，日后可加法式扩展）。日规则为「每月某日（1–28）」或「月末」——「每月 31 日」没有确定的二月，月末应明确写成月末规则。历史补提是一次**显式**操作：用户给出起止窗口，系统按最早优先、每次至多十二个到期日补提，且窗口不得早于该计划授权的生效日；扫描本身只处理当天或之前的**最近**一个到期日，绝不自行回补错过的期间。暂停只阻止未来接收，不取消已接收的工作（取消已接收工作是 Work 自身的独立操作）；结束是终态，不能恢复或再修订。没有任何「全局开启自动执行」开关：授权属于每一个计划自己的指令。
...
| 工资与存货相关会计 | 可记录已提供的工资法定义务及定期存货调整；不等于完整工资处理或永续库存系统。#643 已交付这条边界的记账侧：期间、方法／义务种类、精确金额、各条腿的科目、图表来源与指示都是**由会计师提供的** particulars，产品只核对它们之间的关系（明细必须说出 particulars 说的事；全零、跨财年、盘点日落在期间之外、科目类别不符、预支科目无在世登记、锁定期间各自类型化拒绝）。不发明任何缴纳率、门槛或员工层面的计算；预支的**冲抵分配**仍属 staff-advance 登记簿（`book_staff_advance_application`），一笔缺失的结算事实不得凭空补上。定期存货调整同时是关账检查 `closing_stock_present` 的真正 producer——此前该检查自称没有工具。三个入口已齐：直接会计表单、上传／引用，以及**对话**——用户在 chat 里说出期间与两个盘点数字，Clara 就能把这笔定期调整排进 Work（它说的是"已排队"，不是"已入账"）；缺少任何一项 particular 时它当场点名问，而不是先排队再补问 |
