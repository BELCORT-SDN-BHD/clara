...
>
> **更正（2026-08-16，两次独立按字节核实）：上面这段「后果」的方向写反了。** 结账分录把收入科目**借记**清零（`0056:2250-2258`），而 0016 的营业额是按 `credit_cents - debit_cents` 求和的（`0016:582-588`）——所以未标记的结转分录会把 `ends_on` 当月的营业额**压成大幅负数**，rolling-12 读数**变低（趋近 0）**，永远不会「虚高」、不会推进假 `crossed`。真实的伤害是反向的：**80% 预警梯（`0016:679`）被永久压制**——一个本该在客户逼近门槛时发声的 watch，在结账之后再也不响。缺陷本身、机制链条（三个点）和「错误的警告，不是错误的账」的爆炸半径评估**全部成立**，只有方向这一句要按本更正读。`PROGRESS.md` 的原始措辞（"rolling-12 turnover reads ~0"）一直是对的。分析全文见 PROGRESS Backlog 的 closing_transfer 条目（Fix A 已推荐，候 sitting 的 R1 裁定）。
>
