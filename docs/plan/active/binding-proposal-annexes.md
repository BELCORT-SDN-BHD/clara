...
> | PR | object | kind | D1? | why |
> |---|---|---|---|---|
> | PR-3 (ruled) | `clara._approve_entry_core(jsonb,uuid,uuid,text,text)` | **body SPLICE** (five counted anchors) | **YES — write-quiesce** | the estate's most-shared audited writer; the file carries its own `clara.runtime_heartbeats` quiesce guard. Pre-image prosrc `d5ab4afc…` (19 431 CHARACTERS — `length()`, not `octet_length()`; 19 444 bytes), measured on a pristine `0001..0155` replay and PINNED in the file's own prestate. The independently measured reviewed post-image is pinned at `9682cb13…` in ONE migration-tail constant: the migration still mints the witness from the LIVE `prosrc`, then its tail refuses unless BOTH that live sha and the witnessed sha equal the independent pin. The battery reads the migration constant rather than copying it. **A SPLICE, never a retype** (PR-0 gate B8): the live body is no single file's text and a retype from `0037` would silently restore the breeding block `0106` §E excised — so the apply counts twelve anti-revert markers on the RECUT body before it commits. |
...
> | PR-3 (ruled) | `clara.control_witnesses` — one INSERT | data | **no** | the inserted sha is computed from the LIVE `prosrc`; the independent reviewed-post-image pin lives once in the migration tail (README deploy contract D2) |
