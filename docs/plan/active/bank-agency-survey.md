...
| **C17** | **NEW at v2 (material M2) — the prosrc / overload pins on the extracted PUBLIC bodies** | `x38-wave-c-b-match.test.mjs:1483-1487`, `:1525`, `:1542-1546` · `x38-wave-c-b-bank.test.mjs:2073`, `:2082` · `x40-wave-c-c-tieout.test.mjs:3053-3072`; the reader is `fnSource` (`a21-helpers.mjs:609-615`, which concatenates same-named overloads only) | **MOVE each lock-order pin to the extracted core and ADD the "the public body acquires NOTHING" pin** — the `0042` precedent already in the same file at `x38-match:1496-1538`. Never delete a pin. `:1525`'s exact overload count goes 2 → 1 with the `/13` drop **in PR-3**, and stays exact. Apply-time twins to re-derive on any re-apply: `0044:5393-5401`, `0057:1790-1815` |
