...
| **G1** | `adjustment_run_due` (`0045:5513`) asserts `_assert_due_read_ctx` FIRST (`0045:5525`); with a null `jwt_sub()` that body admits **only** `clara_runtime` (`0042:441-451`, raise `:447`), and the wake pool is `clara_wake_write_login` → `clara_wake_interactive` with no JWT (`pools.mjs:58`, `:373`). B13's ADJ half **raises CLR03 inside the freezing transaction**. ~~`depreciation_run_due` (`0041:3617-3630`) is unaffected~~ **FALSE AT THE LIVE FRONTIER — re-cut 2026-08-27:** `0042`'s WDB-R1/WDB-R2 recut (owner ruling 2026-08-03) replaced that null-tolerant guard with `perform clara._assert_due_read_ctx(v_firm)`, so the FA oracle carries the SAME admission and raises CLR03 from a wake session too. The cite was a superseded-body cite. Both halves of OQ-9(a)'s remedy therefore ship: an additive ungranted `_adjustment_run_due_core` AND `_depreciation_run_due_core`, each below its own live verb's unmoved admission. | §3.6's B13 recut · **OQ-9** · D-26 |
