...
| Row | State | Evidence |
|---|---|---|
| **AC1** particulars + term law + zero refused | **done** | `0207 §B` `_assert_accrual_particulars`; cells `p652.basis.required`, `p652.basis.silent_term`, `p652.basis.zero`, `p652.accounts.roles` (all pass) |
...
| **AC6** the real journey, all states | **done** | `accrual-walk.spec.ts` 14/14 + `accrual-form.test.tsx` 11/11 |
...
| File | What it proves |
|---|---|
| `packages/db/tests/accrual-adjustments.test.mjs` (+ `-fixtures.mjs`, `-preintegration-gate.mjs`) | 17 cells: required particulars, silent term, stated zero, one-commit atomicity (with SQL fault injection), configuration-vs-occurrence, future authority, the orphan wall and the reversal binding, locked-period catch-up, role floors + no cross-firm oracle, account roles, the whole lineage join + the composite FK by name, the grant matrix, entrance independence, the nested reservation, the two plan paths' equivalence, the OBO door on a real `clara_runtime` connection, and the document-anchored term |
| `packages/runtime/tests/accrual-basis-unit.test.mjs` | 16 cells over `lib/accrual-basis.ts`: `.strict()`, the one-literal `term_source`, the closed method set, the local refusals' tokens, the emitted shape |
...
| `apps/web/lib/work/accrual-draft.test.ts` | 21 cells: scope key, op-key lifecycle, untrusted parse, the field mapper over both prefixes, the local validation, the wire shape |
| `apps/web/components/accruals/accrual-form.test.tsx` | 11 cells: viewer DENIED face, both boundaries persistent, focus on first invalid, server field → focused control, refusal verbatim, CLR04, lost-response same op key exactly once, draft across remount, derived lines disabled, accepted → own address, overlap advisory |
...
| Command | Result |
|---|---|
| `node --test --test-concurrency=1 $GATES tests/accrual-adjustments.test.mjs` (29 `--import` gates from `packages/db/package.json`) | **17 tests / 17 pass / 0 fail / 0 skip** |
| `node --test --test-concurrency=1 tests/operation-census.test.mjs` | **10 / 10 pass** |
| `node --test --test-concurrency=1 tests/rig-isolation.test.mjs` (no reset flags) | **31 / 29 pass / 1 fail (T10b) / 1 skip** — see "unverified / rig state" |
| `node --test tests/accrual-basis-unit.test.mjs` | **16 / 16 pass** |
| `node tests/accrual-e2e.mjs` (PG env + `WORKFLOW_POSTGRES_URL` + `RELAY_TEST_MODE=1`) | **PASS 1 · PASS 2 · PASS 3 · PASS 4, exit 0** |
| `node scripts/run-tests.mjs` (whole `apps/web`) | **3751 tests / 3749 pass / 0 fail / 2 skipped, exit 0** |
| `pnpm --filter @clara/web e2e accrual-walk` on 3320/3321/3322 | **14 passed (1.0 m), exit 0** |
...
`packages/db/README.md` (§Migration and deployment behavior: 0207's own consumer-first note and the bare-FK-plus-trigger reason; §Operation-contract census: the four new public names and their cohort) · `packages/db/tests/README.md` (§Running: the focused-vs-sweep gate for this battery) · `packages/runtime/README.md` (§Standalone e2es) · `apps/web/README.md` (route table + why `/accruals` is a top-level segment) · `CONTEXT.md` (**Accrual adjustment**, **Accrual reversal**, **Service period**, **Calculation method**, house "term / _Avoid_" shape).
...
- **Execute**: v18 client pin → `const local = localAccrualRefusal(input); if (local) return local;` → `stableOpKey(ctx.taskId, START_ACCRUAL_WORK_TOOL, input)` → ONE query:
  `clara.create_accrual_adjustment_for($1 ctx.clientId::uuid, $2 ctx.createdBy::uuid, $3 input.purpose::text, $4 {kind:'accounting_work', id: input.authority_work_id}::jsonb, $5 accrualFromInput(input)::jsonb, $6 input.frequency::text, $7 input.day_rule::text, $8 input.day_of_month ?? null::int, $9 'Asia/Kuala_Lumpur'::text, $10 input.effective_from::date, $11 input.effective_to ?? null::date, $12 opKey::text)`.
- **Refusal → message**: hand the database's typed `(code, detail.reason)` back unchanged. Tokens: CLR10 `invalid_accrual` / `silent_term` / `accrual_zero_amount` / `accrual_method_unsupported` / `accrual_account_relationship` / `accrual_term_document_mismatch` / `invalid_purpose` / `invalid_op_key` / `invalid_source_ref` / `invalid_schedule` / `authority_ref_invalid` / `authority_ref_unresolved` / `client_inactive`; CLR11 `client_not_found`; CLR04 `authority_lost` / `insufficient_role`; CLR13 `operation_in_flight`. None is a new error CLASS — `claraWork.v1.errors.ts` needs no change.
...
3. **`method` is `{"rule": <enum>}` with exactly one key**, spelled in the CHECK as `method - 'rule' = '{}'` because a CHECK may not carry a subquery. Four rules, each a selection among stated amounts.
...
6. **`accruals` moved off the `plans` ⌘K row's keyword list** onto its own row — the row that answers the word now shows the service term.
...
4. **#640 SHOULD-1 is inherited, not fixed.** A frequency change can double-cover a period across alignments (`reports/640-review-closure.md` §B). An accrual rides the same arithmetic. Noted, not touched.
5. **Three scheduled-adjustment carriers still only warn.** `_plan_overlap_warning` (0193:1155) is advisory and this lane renders it persistently rather than adding a refusal; the legacy 0045 belt and 0140 §A2's extension can still overlap an accrual. Unchanged by design.

## Unverified / rig state

- **`packages/db/tests/rig-isolation.test.mjs` T10b fails on this rig.** It reports `clara_agent_ro` / `clara_wake_*` reaching seven `graphile_worker.*` functions. **This is rig state, not 0207**, and the proof is direct: the same suite on the same database was **31 / 30 pass / 0 fail / 1 skip** *before* I ran `pnpm --filter @clara/runtime exec bootstrap` (required by `accrual-e2e.mjs`), and **31 / 29 pass / 1 fail** after. No migration creates the `graphile_worker` schema (`grep -rn "create schema[^;]*graphile" packages/db/migrations/` → nothing) and `0207_accrual_adjustments.sql` does not mention it (`grep -c` → 0). Filed as follow-up 3.
