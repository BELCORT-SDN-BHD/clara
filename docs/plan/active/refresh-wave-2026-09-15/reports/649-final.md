...
- **DB battery, 29-gate chain** (`node --test --test-concurrency=1 $GATES tests/client-onboarding-identity.test.mjs`, gates copied verbatim from `packages/db/package.json`): **13 tests, 13 pass, 0 fail, 0 skipped** (9 as first delivered + two round-1 cells + two round-2 cells). **Red first**: the identical file before 0204 → **9 fail**, every one on "the 0204 cohort is required for a focused run"; the two round-1 cells red on the shipped 0204 (`40P01 deadlock detected`; `22003 value "99999999999999999999" is out of range for type integer`); and the two round-2 cells red on the round-1 body — `p649.settle.opening_rung_order` shot the rung-holding transaction with **40P01**, and `p649.settle.foreign_plan_takes_no_lock` had the outsider abort **55P03 after 1046 ms** under `lock_timeout = '1s'` instead of refusing CLR11.
...
- `operation-census` **10/10**; `rig-isolation` (no reset flags) **21 tests, 20 pass, 1 skip** (the destructive T19 cell, as RIG.md requires). Re-run together after the round-1 migration change: **31 tests, 30 pass, 0 fail, 1 skipped**.
- Re-measurement chain: `wb-o-lifecycle` + `knowledge-onboarding-promotion` + `coa-template-pr-b` → **75 tests, 75 pass, 0 fail**.
- **Runtime World legs, both on this rig, both exit 0**: `node tests/interview-e2e.mjs` and `node tests/interview-kill-resume-e2e.mjs`. **Re-measured in round 1** (`PGDATABASE=clara_rt_test` on 55508): `IV_EXIT=0` with `PASS (a)`, `PASS (a-firm)`, `PASS (positive)` and `PASS (p649.interview.sst_park_today)`; `KR_EXIT=0`. `check-parts-parity.mjs` **OK**; `check-frozen-workflows.mjs` **OK** (281 frozen files).
- **Whole `apps/web` suite** (`node scripts/run-tests.mjs`): **3748 tests, 3746 pass, 0 fail, 2 skipped** (after round 1, with its one added cell: **3749 tests, 3747 pass, 0 fail, 2 skipped**, exit 0) — both pre-existing live-provider-gated auth cells. `thread-live-clarify.test.tsx` did **not** flake in the whole run and is **2/2** in isolation. #707/#693 not touched.
- **Playwright** `client-create-walk`: **4 passed (59.7s)**, exit 0. **Round 1 also ran `agentic-finish-walk`**, which owns the H-51/CB-AE2E-024 arm this ticket claims: it was **RED** on the delivered branch (the lane mock left `client_identity_candidates` unanswered for every name outside its own three) and is green after the mock fix — see `reports/649-fixround-1.md` for both runs' counts.
