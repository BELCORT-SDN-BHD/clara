...
| **OC-6** | **RULED 2026-08-30 (裁-39) — ALL EIGHT, each per the Annex D default.** OQ-1 staff allowances get their **own contract item**, not F-T2 · OQ-2 Form E shows the **statutory 31 March** with the e-Filing grace **annotated** · OQ-3 the **CP21/22 family is IN** as reference rows · OQ-4 **CP58 stays and reads `unknown` when the fact is absent, never a verdict** (this **overrides OC-3's provisional unconditional "applies"**) · OQ-5 the payroll-JV roles come from **the COA template's `statutory` marks** (裁-21) · OQ-6 the HRD Corp conflict shows **both dates flagged `conflict`**, the earlier governing · OQ-7 **EPF rolls forward to Monday, the rest back to Friday, public holidays explicitly unhandled** (this **closes OC-4** in the same direction its default already took) · OQ-8 **no fourth Tier-1 rate table**. Ledger: `mohe-grill-rulings-2026-08-29.md` §裁-39; per-question text in `payroll-calendar-annexes.md` Annex D. *(As put, pre-ruling: eight questions carried unchanged from v1, each proceeding on its Annex D default.)* | **Now ruled** — the defaults are the rulings. | **F-T2's rows are unblocked** |
