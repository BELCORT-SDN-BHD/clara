...
| W45 | **the replay identity rides an INJECTIVE, TRANSFORM-STABLE encoding — never a display string** (added at the 2026-08-28 fix round, from the review lane's still-partial finding on the widened identity) | four arms + the freeze: **(a)** a rationale-only change under the same key refuses with the TYPED `op_key_reused_with_different_args` token; **(b)** a **delimiter-straddling** pair — `r1 = X`, `b1 = Y \| target account <a>: Z` against `r2 = X \| target account <a>: Y`, `b2 = Z` — refuses, where the composed-rationale identity made the two byte-identical and the second replayed SILENTLY; **(c)** a **near-ceiling** rationale (3983 chars, under B2's 4000 bound but over it once composed) acts and its **identical retry REPLAYS**, where the old comparison met the receipt's `left(...,4000)` copy and false-REFUSED; **(d)** the identical-request control stays green. Plus `W45-frozen`: the persisted digest is UPDATE-refused by the transition trigger even as root | **the injectivity mutant runs in the DB, before the arms**: the same straddling pair is composed BOTH ways and asserted byte-EQUAL under the old encoding and digest-UNEQUAL under the shipped one — without it the cell could pass against an identity that never had the defect. **(c)**'s own control measures the receipt at exactly 4000 characters, so the truncation it defends against is proven present rather than assumed |
