...
| W39 | **F2 — the three walls** | (1) an ineligible target (bank-class, non-P&L, or absent from the COA) → `prepayment_target_ineligible`; (2) the acted receipt carries the account AND its basis; (3) an admin signing a CHANGED account posts to the changed one, and `content_hash` covers it ▣ | pass a well-formed expense account → acts, so wall (1) is not refusing everything |
