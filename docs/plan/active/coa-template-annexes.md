...
| **PR-c** | **SPLIT 2026-08-31 (the beta pivot, 裁-79/裁-80).** **BUILT (#463):** D-7's interview changes riding `clientOnboarding_v4` — the consumption half only, no trim. **PARKED, not scheduled:** `wake_propose_coa_template_trim`, its allowlist row, the receipt write — 裁-77's admitted backend list does not name this verb, so it does not build pre-beta | **none** (a workflow deploy, not a D1 window) | #463 (built half): full ladder + independent pass, no injection surface without the trim. **The parked half, WHEN built:** full ladder + independent pass + **law 28's cross-model adversarial pass — MANDATORY** (injection surface: model-proposed family names) |
