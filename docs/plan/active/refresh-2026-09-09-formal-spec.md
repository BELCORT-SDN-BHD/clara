...
PRD is the enduring human-readable product blueprint: vision, users, problems, behavior, core journeys, scope and completion conditions. Architecture is the enduring technical blueprint: stack choices and reasons, boundaries, module responsibilities, dependencies, data flows and trade-offs; it distinguishes verified current implementation from explicitly accepted target architecture. Domain terminology stays in CONTEXT. Update these blueprints after accepted Wayfinder or spec decisions and before ticket decomposition, then update current-state claims as implementation is verified. This formal GitHub spec and its appendices are a dated delivery contract; later accepted specs may supersede delivery scope without making an old spec the permanent product authority. Implementation issues own dependency/status/verification. Research snapshots retain their historical evidence limits and are not rewritten to imply unproven delivery.
