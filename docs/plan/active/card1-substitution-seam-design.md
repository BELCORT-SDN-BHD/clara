...
> **v2 — gate-folded 2026-08-26.** The 2026-08-25 gate returned **fold-required**: 8 blockers,
> 12 material, 9 nits, 10 battery gaps. **All 8 blockers and all 12 material findings named
> with content are folded into this v2** (BL-1..BL-8, M1..M12 — the fold-confirmation table is
> in this branch's commit message and the coordinator's own report). Two nits (**N2, N6**) were
> relayed with content and are folded; **the remaining 7 nits and 9 battery gaps were referenced
> by count in the relay but not relayed with content, and could not be folded — flagged for the
> coordinator, not silently treated as satisfied** (review law 2: absence of content is not
> evidence the finding was addressed).
>
...
for inline resolution). The `placeholder` case, **widened per BL-8**:
...
- **`clara.claim_sandbox_export(p_worker text, p_lease interval default interval '20
  ...
  e.created_at, e.id` — `sandbox_exports` has `created_at`, not `render_jobs`' own
  `enqueued_at`), the retry cap enforced **in the claim predicate itself**
  (`e.attempts < e.max_attempts`), lease bounded to `[1 minute, 6 hours]`, and the SAME stamps:
  `state='running', claimed_by, claimed_at=now(), lease_expires_at,
  attempts=attempts+1, first_claimed_at=coalesce(first_claimed_at, now()),
  claim_delay_ms=coalesce(claim_delay_ms, (extract(epoch from (now()-created_at))*1000)::bigint)`.
