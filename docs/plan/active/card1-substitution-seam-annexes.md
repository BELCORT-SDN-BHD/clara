...
> **v2 — gate-folded 2026-08-26**, alongside `card1-substitution-seam-design.md` +
> `-part2.md` + `-part3.md` (the design of record, now three files). Companion to
...
>
> **This v2 folds BL-1..BL-8, M1..M12, N2, N6** — the fold-confirmation table is in the
> branch's commit message. The remaining 7 nits and 9 battery gaps referenced by count in the
> relay carried no content and could not be folded — flagged for the coordinator in the design
...
| B1.4 | **the mixed-body widening cell** — a body with one `placeholder` block (citing an `'ok'` preview cell) and one `text` block (citing anything) → `client_set_basis='firm_closure'`, NOT `'exact'`; the twin body with the `text` block removed (placeholder-only) → `client_set_basis='exact'`. This is S30's boundary made a differential |
| **B1.5 (N2)** | a body made ENTIRELY of `placeholder` blocks, each citing a distinct client's cell → **asserts against `client_set_exact` specifically** (`0132.sql:709-716`'s NT-1 field, returned separately from the widened `client_set` precisely so this assertion is meaningful — asserting only against the widened `client_set` would pass even on a silently narrowed exact derivation, mirroring the parent lane's own B1.9 concern), derives the exact union of those clients |
...
| **B4.5 (M6)** | **the context-mismatch cell, period axis** — a `cell` node citing a definition-backed, `'ok'` cell computed over periods `{P1,P2}`, composed inside a formula whose own context binds `{P1,P3}` → REFUSE `metric_cell_context_mismatch`; the twin with matching period sets succeeds |
| **B4.6 (M6)** | **the context-mismatch cell, watermark axis** — a `cell` node citing a cell minted against `books_watermark = W1`, composed inside a formula whose snapshot's `books_watermark = W2` → REFUSE `metric_cell_context_mismatch`; the twin with matching watermarks succeeds |
...
| B4.9 | the twelfth primitive census: `select count(*) from clara.metric_primitives` reads `12` after this migration, never `11` — **and, per BL-1, an insert of `'cell'` attempted BEFORE the ALTER runs (against a fixture snapshot of the pre-ALTER constraint) fails**, proving the ALTER — not the insert alone — is what makes the twelfth primitive admissible |
...
| **B5.6 (BL-3)** | **the deploy-ceremony cell, both polarities.** `UPDATE clara.evaluator_versions SET deployed=true WHERE evaluator_name='evaluate_metric' AND version=2`, run under a session with `SET ROLE clara_fn_owner` active (`current_user <> session_user`) → REFUSE `'evaluator deployment requires the migration ceremony principal'` (CLR08); the twin, run under the bare migration-runner principal with no `SET ROLE` active → succeeds, and `wake_compose_metric_preview_v2` — which refused `evaluator_undeployed` before the flip — succeeds after it, with nothing else in the estate changed |

### B.6 · The claim/dispatch verbs and the ALTER (design §2.6, BL-6)

| cell | forces |
|---|---|
| **B6.1 (BL-6)** | every NEW mutable column of `sandbox_exports` (`first_claimed_at`, `claim_delay_ms`, `dispatch_attempts`, `last_dispatch_at`, `last_dispatch_ok`, `last_dispatch_error`) accepts its lawful transition through the recut `_tf_sandbox_export_lifecycle`'s widened `mutable` array; `max_attempts` — deliberately NOT in the mutable array — REFUSES an UPDATE attempting to change it, mirroring `render_jobs`' own frozen-request-half precedent |
| B6.2 | `claim_sandbox_export` transitions a `claimable` row to `running`, sets `claimed_by`/`claimed_at`/`lease_expires_at`/`first_claimed_at`/`claim_delay_ms`; a second concurrent claim attempt on the same row (simulated via `FOR UPDATE SKIP LOCKED` under two sessions) never double-claims; a row at `attempts >= max_attempts` is never claimable, even with an expired lease |
...
| **CD-14 (BL-2)** | `clara.evaluate_metric_v2` is minted as a real, callable, correctly-hashed entrypoint — the "honest branch" satisfying the freeze machinery directly — but `propose_metric_definition`/`approve_metric_definition` stay v1-scoped; no canonical, `cell`-referencing definition is buildable through this session | design §3.2 item 4; this design's own scope-boundary judgement call, flagged for sign-off |
...
| **R-CD-5 (new at the fold)** | the render-time `need()`-shaped mirror (B3.2/B3.3) depends on the payload builder's `cells` object using the SAME label keys the mint-time `basis` array used, AND on M11's filter (`b.label in (select ... where kind='placeholder')`) staying in sync with the `_sandbox_client_set`'s own placeholder-basis-ref validation — a drift between the two filters (one loosens, the other doesn't) would produce either a false `sandbox_cell_unresolved` or a silently over-broad payload | a battery cell asserting the two filters' label sets are identical on the same fixture view is owed alongside B2.3/B3.2 |
...
8. `claim_sandbox_export`/`sandbox_dispatch_begin`/`sandbox_dispatch_record`/`reap_exhausted_
   ...
   path a live leader loop would (B6.2-B6.5).
