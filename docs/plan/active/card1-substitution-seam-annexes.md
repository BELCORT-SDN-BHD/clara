...
| **CD-9 (BL-4, new)** | The AST document tag `clara.metric/v1` and the composition schema tag *clara.metric-composition-inputs/v1* (a contract name, not a path) stay UNCHANGED — the grammar widens, the document format does not; vocabulary identity is carried by the EVALUATOR VERSION `(evaluator_name, version)`, never by a document-format string | design §3.1; S21's own `(name, version)` identity model |
