...
| **PR-1** | **`sst_rate_schedule`** + seed · the reachable-closure write assertion · the **`PRD.md:215` prose→table** correction. ⚠ **The `sst_threshold_schedule` widening is CUT from this row pending OQ-14** — `internet-lane-design.md:431` still assigns the identical ALTER to F-A8/PR-3, and whichever lands second collides. Annex A.1 keeps the specification | **none predicted** on the rate table alone | **OQ-14 ruled** before the threshold limb may re-enter |
...
| **PR-6** | **`sst_returns` / `sst_return_lines` / `sst_return_02a`** + the **producer** + the per-field mapping + NIL + §4's refusal + §3.8's monthly SST-02A calendar | none | PR-4, PR-5, **OQ-11 ruled** — without a value-side producer and a rate-bucket rule, **no non-NIL return is materialisable** and the mapping has no inputs |
