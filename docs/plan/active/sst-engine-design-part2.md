...
| **PR-2** | **`sst_registrations`** (incl. the opaque s.11(1A)/s.25(3) approval trio) + **`sst_taxable_periods`** + the period generator + the `statutory_deadlines` **SST seed rows and consumer** | none predicted; the seed rows land in **F-A4's** table | **F-A4's `statutory_deadlines` DDL merged** — *as at 2026-08-23 no F-A4 PR carries it* |
...
| **PR-4** | the **payment-basis evaluator** · **`service_period_start`/`_end` on the AR open item** (§3.3) · CN/DN under reg 11 incl. the `unallocated_credit_forbidden` wall · `sst_deferred_realisation` + s.11(2) **return-only** | ADD COLUMN on `open_items`, nullable, no backfill — **none predicted**; the table is append-only by trigger (`0037:823`), so verify the ALTER against it | PR-2, PR-3 |
...
| **later** | s.11(2)'s **ledger** arm + §4's shape A, if the owner rules them in (OQ-3, OQ-4) | **D1** — CoR of `_assert_sales_invoice_shape_at`, shares F-A2's window | **F-A2 PR-1 merged and settled** |
...
must not become the return's basis · **R3** the payment-basis deferral collides with F-A2's live wall ·
