...
| **A33** | **NEW at PR-3's review rounds — C2's per-subject-account digest binding is NOT landed, for ANY of the thirteen agent bank cores.** `_agent_verify_inputs_digest` binds a pack-read's digest to the acting TASK (`split_part(op_key,':',2)`) only; it does not additionally bind to the SUBJECT (account) the pack read was about. Probed on the live catalog (`pg_get_function_arguments`) against the three cores an earlier round's seam message guessed were "trivially derivable": `_agent_add_bank_account_core` takes `p_proposal_id` (a bank_account_proposals row, not an existing account — it CREATES the account), `_agent_upsert_account_core` takes no account-shaped uuid parameter at all (keyed by `p_code`), `_agent_void_bank_statement_core` takes `p_statement` (a statement id, reachable to its account only via a join) — **none of the thirteen carries a directly-named bank-account-id parameter**, so the "3 of 13" premise itself does not hold; the deferral covers all thirteen, not ten. **Acceptance criteria for a future PR to close this:** (1) for each of the 13 cores, name the concrete derivation path from its own parameters to the `bank_account_id` it acts on (a join query per core, rig-measured, not asserted); (2) `_agent_verify_inputs_digest` gains a subject-binding parameter and requires the cited pack-read's own `subject_id` to match the act's derived account, same-task binding alone no longer sufficient; (3) a same-task, cross-account negative cell per core that HAS a derivable account, proving a stale digest for account A no longer grounds an act on account B within the same task. Closes the narrower same-task cross-account leak; the cross-task leak this round already closes stays closed either way. | **deferred — Codex final leg C2-bis, probe-confirmed (0129, review round 4)** |
