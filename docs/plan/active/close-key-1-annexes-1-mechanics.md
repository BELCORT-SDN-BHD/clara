...
- **B13 `belt_period_unrun`** *(re-cut at gate 2 — GM-3 + G1)*. Three arms, evaluated in this
  ...
  1. **FA due.** ~~`clara.depreciation_run_due(p_client)` is wake-safe (`0041:3617-3630` compares a
     non-null `jwt_firm()` only, and `jwt_firm()` is null here)~~ **RE-CUT 2026-08-27: it is NOT
     wake-safe.** `0042`'s WDB-R1/WDB-R2 recut gave it `_assert_due_read_ctx`, so this arm reads
     the additive ungranted `clara._depreciation_run_due_core` (OQ-9(a)'s shape, applied to the
     sibling) exactly as arm 3 reads the ADJ core — refuse when
...
| C-19 ▣ | **the positive control G1 was found by lacking** — call `clara.adjustment_run_due(client)` through a REAL `clara_wake_write_login` → `clara_wake_interactive` session (the write pool's own path, `pools.mjs:373`): pre-fix it raises **CLR03 `no valid read context`**; post-OQ-9(a) the ungranted `_adjustment_run_due_core` answers, and the live `adjustment_run_due` **still raises CLR03** on that session (the admission is unmoved for every existing caller). ~~Twin: `depreciation_run_due` answers on the same session both before and after~~ **RE-CUT 2026-08-27: it answers on NEITHER** — `0042` gave it the same admission, so the twin now asserts that BOTH live oracles are unreachable from the wake lane and BOTH ungranted cores answer. Note also that `clara_wake_interactive` holds no EXECUTE on either verb, so the ACL refuses **42501** one rung earlier than the admission's CLR03; the cell asserts the disjunction and records which fired (battery cell `fa4c.F1`) |
...
| C-17 ▣ | **the belts are byte-untouched** ~~for all four names~~ **— RE-CUT 2026-08-27: `depreciation_run_due(uuid)` and `adjustment_run_due(uuid)` are BOTH thin delegates now (OQ-9(a) and its FA sibling), so the prosrc pin holds for the two RUN verbs only; for the two oracles the pin is on their ADMISSION and their grants instead (§TAIL T.6 counts all four EXECUTE grants).** `to_regprocedure` on `depreciation_run_due(uuid)`, `adjustment_run_due(uuid)`, `run_depreciation_period(uuid,date,date,text)`, `run_adjustment_occurrence(uuid,uuid,date,date,text)` and the `prosrc` of each read exactly as at `cfa0710` |
