...
> Companion to `freeform-read-v2-design.md` (the design of record) and
> `freeform-read-v2-survey.md` (the estate at the bytes). **Where this file and the design disagree,
> the design is right and this file is the bug.** Where this file and the **migration's printed
> line** disagree, the printed line is right — the standing caveat v1's A.2 minted for exactly this
> class of drift.
...
| 11 | `p_documents_freeform` | **RE-CUT** onto S-2e's shape comparing `documents.id` | Y5 — `documents.client_id` is frozen at ingest and the correction path cannot move it |
...
| 14 | `CLR10 cross_client_unavailable` + its message + its battery cell | **RETIRE, all three together** | design §7 |
...
and it is *useful*: a HOME read that names its clients produces a receipt with an **exact**
...
| `cross_client_unknown_client` | A (`CLR11`) | an element is not a live client of `wake_firm()` |
...
| B1.3 | a removed/inactive client → refuses; the twin with a live client succeeds |
...
| B1.7 | duplicates and unsorted input produce the **same** canonical `client_scope_set` and the **same** `op_key` |
...
| B3.5 | `documents` and its extractions agree **after a refile**: B sees both or neither, A sees neither. The incoherence Y5 predicted, forced in both directions |
...
| B3.7 | the two-hop arm does not degrade to a seq scan at ≥100k regions (Q-2) — **measured, and the number recorded**, never asserted |
...
| **C-8** | **`documents` is re-cut onto the filings join** | Y5 — `documents.client_id` is frozen at ingest (`0003:402-406`); one rule for the whole document band |
...
indexed at scale — **if not, the cost is priced at PR-1, not absorbed** · **Q-3** `= any(uuid[])`
...
| **R-3** | the **two-hop policy** is a performance wall at scale (Q-2) — a security-shaped bug that presents as a timeout | measured at PR-1 with a real-size fixture, and the number is recorded in the acceptance record whether or not it is good |
| **R-4** | **Y5's v1 defect is not routed** and v1 ships `documents` scoped by a frozen column — then v2's re-cut looks like v2 breaking v1 rather than fixing it | survey U3 routes it to the lead **today**, before v1's PR-1 is authored |
...
8. The full estate suite is green on a pristine rig, tails unfiltered, **every skip named and
   counted**, with Q-2's measured number recorded whatever it is.
