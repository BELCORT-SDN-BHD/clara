...
| 10.6 `get_llm_usage_summary` — in-catalog callers, ACL, owner's answer | pre: no in-catalog caller; post: one, `get_firm_ai_usage` (0233's own door). EXECUTE unchanged: `clara_authenticated`, `clara_fn_owner`. `track_functions` = none, so call counts are unknowable. **Owner's answer: pending** — asked in the release report |
