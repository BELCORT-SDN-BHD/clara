...
| /onboarding/client — begin/resume | `begin_client_onboarding`, plan reads | `apps/web/components/clara/OnboardingChecklistCard.tsx` (`apps/web/lib/onboarding/api.ts`); resume via `components/firm/client-register-list.tsx:64`'s onboarding badge | SAME (relocated) |
| Interview attachments (sample invoices side-channel) | shared upload queue | `apps/web/components/clara/InterviewRunCard.tsx` (confirmed: a real `type="file"` input exists, `:301`) + `apps/web/lib/interview/api.ts` | SAME |
...
| `/opening` — create/cancel/reopen seed, draft items, fixed-asset seeding, approve/correct, dry-run, KEYED fallback (`record_opening_target`) | ~9 doors | `apps/web/components/registers/opening-*.tsx` + `apps/web/lib/registers/opening-doors.ts` — **every RPC name has a live caller** | SAME (relocated to client-workspace Registers tab, Q3 IA) |
...
| `/seeding` — prior-GL tick-list (batches, proposals, tick/decline, complete/cancel batch) | 6 doors | `apps/web/lib/reports/api.ts` (all present), `apps/web/components/reports/SeedingBatchesPanel.tsx`, `apps/web/components/firm/seeding-proposal-affordance.tsx` | SAME (relocated to Reports tab, Q3 IA) |
