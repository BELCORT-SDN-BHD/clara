...
| `reconcile_autopost_rules()` | `0015:2759-2795`; drift pin `0017:410-434` | **`0015:3566-3568` → `clara_runtime` (the GROUP)** — a *different* posture from the executor's login-direct; do not conflate | **CALLER RETIRED** — the DB function dropped at `0118` (this row) but the caller (`reconciler.mjs`'s `reconcileAutopostRules` belt + `leader.mjs`'s `autopostReconcileDue` cadence knob) was missed in the original PR-3 pass and kept firing the dropped call every ~2s poll until a follow-up PR unwired it in full; the original line-number citation here (`lib/leader.mjs:79,177,185,191` → `lib/reconciler.mjs:622,472,475`) never resolved to the right lines even at design time — read the shipped diff, not a citation into a file that keeps growing |
