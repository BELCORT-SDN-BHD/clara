...
- `grant_client_egress`, `revoke_client_egress`, `claim_document_processing_task`,
  ...
  - **A9 exception (2026-07-28, task #27)**, a SECOND deliberately-changed member,
    landed in migration 0025 rather than in 0020 itself: `_enqueue_invoice_facts_core`'s
    kind-list literal `('invoice','credit_note','debit_note')` widens to
    `('invoice','credit_note','debit_note','receipt')`. Its pin is likewise **not**
    retuned — the tail reverses exactly that one substring and re-hashes the remainder
    against 0020's own untouched prestate, so the assertion proves the widening is
    present in its exact shape **and** that nothing else in the body moved (including,
    crucially, 0017's own O8.6 inactive-client patch to the same function — a wrong-base
    CoR would fail this reversal too, not just the final hash).
