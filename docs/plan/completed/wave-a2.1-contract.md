...
| WA21-R13 | **The wave CLOSES on W/C/D (owner ruling, 2026-07-23; ADR-030); Gates S and P split out as FOLLOW-ON eval items deferred to real documents.** S structurally cannot run at close: zero credit sightings exist (the side-aware pool starts empty by design — the historical sales approvals predate 0016 and backfilled `debit`), the structured floor needs ≥3 post-0016 human-approved sales entries, and no MyInvois XML exists yet; P has no unused SST-stated bill. Both close on the next real document cycle — **never on synthetic documents** (the ADR-027 doctrine's spirit: deferral beats fabricated evidence on live books). Gate D's customer-vocabulary + settled-card-reload checks ride the S follow-on session. |
