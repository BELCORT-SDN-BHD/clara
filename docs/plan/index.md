...
| `f-a1-witness-pair-design.md` | **live** | **The F-A1 design doc of record** (v3, 2026-08-18): the witness-pair mechanics under C1-C4 (cite-and-verify snap, atomic idempotent two-kind persist, `evaluate_witness_fact_state_v1` in the central evaluator freeze, resolver dispatch sparing ~30 call sites, the five-list egress wiring + the `witness_extraction` typed purpose, meter-never-cap), the D1-D12 decision register, OQ-1/OQ-2 for the sitting, and the PR-0..PR-4+3a build sequence. Two-lane adversarial review to unconditional MERGEABLE (48 findings); **build gated on the Codex PR-0 pass** (vendor limit lifts 2026-08-20). |
