...
- **Never dispatch the main model.** Every subagent/workflow/teammate dispatch carries an explicit `model` (`opus` for judgment lanes, `sonnet` for mechanical); omission silently inherits Fable, which is forbidden (owner ruling 2026-07-23; the owner deliberately does NOT set the `CLAUDE_CODE_SUBAGENT_MODEL` force-cap — dispatch stays flexible, so the explicit per-dispatch `model` IS the enforcement; see memory `feedback-no-fable-subagents`). Codex lanes stay `gpt-5.6-sol`.
