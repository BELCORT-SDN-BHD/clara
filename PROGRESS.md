...
wait for runtime **v72** + the statement-lane fixes + the "Enter a statement" form fix + **DPA v2**.
...
and its [part 2](docs/plan/active/beta-handover-2026-09-04-part2.md).** That is the complete report
...
- **Runtime:** Fly machine `48ee715b763048`, 2/2 checks, `/ready` **true**, `held_outbox` **6** at the
  close (0 at 02:39; cause not investigated — handover row H-44). `fly status` VERSION reads **74**,
  which is the machine's config counter after the 裁-179 secret imports — **the IMAGE is still v71,
  built from `344f7ad8`**. **#533's reconciler unwire is MERGED AND NOT SERVING:** the DB no longer
  defines `clara.reconcile_autopost_rules`, v71 still calls it, and the runtime logs the error every
  ~2 s. **v72 is handover row H-01 and it is the first thing to do.**
...
- **Estate:** firms **Alara · Borneo · BELCORT** (`04daf86c-3aaf-4c59-9442-cce93f3582af`,
  ...
  (active, 86-account chart, one sales invoice posted, one bank receipt settled, FY2025 open).
...
1. **The P0 block — everything the beta stays CLOSED until.** v72 (H-01) · the "Enter a statement"
   ...
   **DPA v2** (H-36) and the Stripe product-description edit (H-37) · verify-full TLS (H-43) and the
   two password rotations (H-42) · the re-migration runbook truings and the per-lane boot probe
   (H-47 · H-48). ≈**6–7 lane-units** plus a handful of owner minutes.
...
and its [part 2](docs/plan/active/beta-handover-2026-09-04-part2.md), each with what was measured,
...
- **P0 — before the first EXTERNAL applicant (14 rows), handover §C.1:** H-01 the v71↔DB skew
  (`reconcile_autopost_rules`, v72) · H-02 Maybank headers state no period bounds · H-03 the
...
- **P1 — before 上市, the walk's own rows, handover §C.2:** H-04 the classifier does not recognise
  ...
  Stripe webhook endpoint · H-40 two undecided Supabase auth settings · H-44 `held_outbox` 6 ·
...
- **P1 — already ruled before the launch night, handover §C.1's second table:** C-01 the pre-上市
  ...
  (裁-165) · C-06 the beta Terms of Service (裁-129/166) · C-07 the XML `blob:` open with no MIME gate
  and no CSP (裁-175) · C-08 the ten 裁-176 ports and fixes · C-09 the checkout/webhook follow-ups ·
...
- **P2 — hygiene, handover part 2 §C.3 · §C.4 · §C.5:** ~14 web nits from the walk (H-10 · H-13 ·
  H-14 · H-22 … H-34 · H-50 … H-53) · the full CARRIED registry C-17 … C-76 (database, runtime,
  frontend, reporting/close/tax, ops/DR/security, harness/CI) · and **§C.5, the seventeen
  documentation truings the rulings ordered that this truing did NOT execute** — the Wave-G checklist
  and `docs/ops/DR.md` lines from 裁-161/162/169/170/177, the T-K wrangler comment, the `0161`→`0163`
  citations, and the rest, each naming its file.
...
- **The serving image and the schema disagree** (H-01) — `reconcile_autopost_rules` is called by v71
  and defined nowhere; the autopost reconcile belt is dead and the log storms every ~2 s. **v72.**
...
- **Host and worktree hygiene:** three locked worktree shells plus one corrupted directory need an
  elevated shell after a Claude Code restart, then `git worktree prune`; the WSL `.vhdx` compaction is
  owed again (~50 GB reclaimable at the last measure). Two standing host facts: the OpenNext/Workers
  artifact must be BUILT ON LINUX, and **WSL idle-terminates without a Windows-side holder** — plant a
  detached keeper before any port-dependent WSL work.
...
- **The confirmation login-CSRF finding — TRUED.** The always-refusing stub is gone; the wall is wired
  ...
  replay siblings (handover part 2, C-63).
...
- **2026-09-04 — BETA WENT LIVE, AND THE SPRINT SESSION CLOSED.** In one continuous sitting from
  ...
  the handover. **Nine of the fourteen P0 rows were found only because the walk used the real product
