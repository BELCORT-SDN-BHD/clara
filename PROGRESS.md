...
1. **THE FRONTEND 磨合 WINDOW opens next (its own dedicated session)** — the Wave-F backend is
   ...
   **PRE-FLIGHT (MEASURED 2026-08-26 in-VM):** serving bundle pins chatTurn_v13/autoDraft_v9; chatTurn_v14 is NOT serving — a D-a-style runtime image deploy carrying v14 is REQUIRED before bank-chat parity is exercisable.
...
- ~~**`coding_rules` propose/sign retirement**~~ — **DONE**: `0118` (F-A2 PR-3, #324) drops
  `propose_coding_rule`/`sign_coding_rule`/`decline_coding_rule`/`retire_coding_rule`/
  `get_coding_rule` outright, confirmed absent by the migration's own tail assertion;
  `coding_rules` itself stays KEEP-AS-HISTORY. Writes stopped, history stays as knowledge
  fuel — the same treatment, consistent with OQ-2's ruling.
- ~~**The autoDraft 8-step cap**~~ — **DONE**: `autoDraft.v9.impl.ts:197` names it
  `AUTODRAFT_STEP_BUDGET = 8`, with the design-cell docstring explaining the bound
  (F-A2 PR-2, #323).
