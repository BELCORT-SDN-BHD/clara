...
141/`0146`**, as-run `docs/plan/completed/mohe-0142-0146-apply-asrun.md` · **hardening B (裁-16) MERGED as `0147` (#414, main `752b8dde`) — live is STILL 141/`0146`, its D1 window PENDING on the sweep going green for the `seedAdmission` fixture fix (branch fix/seed-admission-frontier; Known issues)** · **THE 2026-08-29 MORNING SITTING IS RULED — 裁-23…裁-28** (`docs/plan/active/mohe-grill-rulings-2026-08-28.md`): the three pre-beta design gates are CLOSED and `docs/plan/active/mohe-owner-batch-2026-08-29.md` is answered except the pricing amounts)*
...
1. **THE FRONTEND 磨合 WINDOW IS OPEN AND RUNNING** — pre-flight discharged, P1-P3 merged,
   ...
   **In flight (2026-08-29, after the morning sitting)**: hardening B MERGED (`0147`, #414), its D1 window waiting on the sweep of branch fix/seed-admission-frontier · **the three ruled builds** — 裁-21's **research lane before PR-0** (裁-23), 裁-19 as a **HYBRID** (the read layer plus an appended re-home pair for OPEN items in UNFROZEN periods, frozen years read-layer only; 裁-24), 裁-18b now **FIVE PRs / two D1 windows** (the post-time re-check restored, the expiry sweep rides; 裁-25) · 裁-26 (admission-token email wall) → **P4's UI tranche**, 裁-27 ("Amend resolution") → **P6** · T8's alias-panel wiring onto `counterparty_aliases_visible` and PR-e's proposal card (P6) · the `0133` operator-flag act for BELCORT (owner-timed). **Still owed in this window** (owner rulings,
...
- **THE SWEEP-RED FIXTURE CLASS (2026-08-29, after #414/`0147`) — a shared fixture that follows
  HEAD breaks the sweep-only legs.** The manual sweep came back RED on the closed-wave drill
  **§4.11** and all four **D-b frontier legs**: the shared `seedAdmission` fixture writes
  `token_hash`, which does not exist at the pre-`0147` frontiers those legs replay. The fixture is
  made **frontier-aware (it probes the catalog)** on branch fix/seed-admission-frontier; **product code
  is unaffected**, and **`0147`'s live D1 window waits for that branch's sweep to be green**.
  **The standing lesson:** the closed drills and the frontier matrix run **only** on the weekly
  sweep or a manual dispatch (ADR-0073), so a PR that changes a SHARED FIXTURE must
  `gh workflow run ci.yml` on its own branch **before merge** — the per-PR legs cannot see it.
