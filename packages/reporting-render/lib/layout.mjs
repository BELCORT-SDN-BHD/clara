...
export function assemble({ layoutAst, payload, decision, style, fonts }) {
  ...
  emit(`#let s(t) = text(t)`);
  // THE MANIFEST'S DOCUMENT METADATA IS APPLIED, NOT JUST DECLARED (codex M11) — and the emission
  // is bounded by what THE PINNED ENGINE CAN CARRY, which is the round-2 correction. An earlier
  // draft also emitted `description:`; that argument arrived in Typst 0.13.0 and this image pins
  // 0.12.0, so typst rejected the preamble of every real document while CI stayed green, because
  // the only thing that ever ran typst was a hand-written fixture that emitted none of this. The
  // drill now compiles THIS function's own output for exactly that reason.
  //
  // WHAT 0.12 CARRIES: title, author, keywords and a date (all since 0.9.0). It offers no facility
  // for a PDF Subject or for the trailer /ID, so the manifest no longer pins either — a manifest
  // that pinned a value nothing could carry would be a claim about a document that does not exist,
  // which is the very disagreement §7(d) exists to catch. If the engine pin ever moves to >=0.13,
  // `description:` is the one-line way to carry subject, and manifest.mjs says the same.
  ...
  function renderChart(n) {
    ...
    // THE TABLE CELL HAS EXACTLY ONE LAWFUL SOURCE (round-2 M4, the sibling of :250-256). This read
    // `r.displayed_text ?? "n/a"`, which put a renderer-authored word for a missing figure into the
    // SEALED artifact's data table — the same defect the metric_ref path had just closed, one hop
    // away and inside the same PDF. A plotted point prints the DB's figure; a gap prints the DB's
    // sealed na_label; anything else refuses, because "what this statement says about a missing
    // number" is a disclosure decision and the renderer does not get to make it.
    const cellText = (r) => {
      const own = r.plotted ? r.displayed_text : r.na_label;
      if (typeof own === "string" && own.trim() !== "") return own;
      throw new RenderRefusal(r.plotted ? "chart_point_text_unsealed" : "na_label_unsealed",
        r.plotted
          ? "a plotted chart point carries no displayed_text; the figure printed beside a bar is the database's string, never the renderer's"
          : "a chart gap carries no sealed na_label; the renderer will not author disclosure text for a missing figure",
        { chart_key: n.chart_key, series_key: r.series_key, ordinal: r.ordinal, cell_id: r.cell_id });
    };
    const rows = table.map((r) => `[#s(${typstString(r.series_key)})], [#s(${typstString(cellText(r))})]`).join(", ");
