...
import { CLARA_WORK_BUNDLE_V7_BANNER } from "../workflows/claraWork.v7.bundle.js";
...
export default definePlugin(() => {
  ...
  // imported v1 step (statementFacts.v4.impl.ts) UNCHANGED, and this bundle is additive rather
  ...
  // deliberate exception and live inside the ACTIVE closure, statementFacts.v4's (design M8,
  // inherited). The MODULE this bundle is built from keeps its v2 filename on purpose: it is
  // shared unchanged by v2, v3 and v4, and renaming it would move a frozen hash for no
  // behaviour — which is also why #1037's repoint needs no boot-side edit beyond this comment:
  // statementFacts mints no bundle banner (only claraWork does), so the class's boot pin is
  // carried entirely by `emitProvenanceLine`'s read of `workflowPins` and by
  // `strandedBodyCensusOnWorld(workflowBodies)`, both of which the registry's five edits move.
  ...
  // #945: payrollFacts_v1's OWN bundle, in its OWN slot. It carries its OWN engine snapshot
  // (`llm-openai:{model}:payroll-witness-v1`) and the frozen behaviour refuses to egress under a
  ...
  void (async () => {
    ...
    try {
      ...
      // CLOSING WAVE 2026-09-26 (#1144) — THE SEVENTH LINE, ADDED IN THE SAME COMMIT AS THE
      // REGISTRY REPOINT, because the comment two above this one is the record of what happens
      // when it is not. v7 is what `workflows.claraWork` now dispatches; v1..v6 are still carried
      // for runs parked on their hooks. `tests/pinned-work-bundle.mjs`'s `waitBooted` blocks on
      // exactly this line for the PINNED version, and its throw is swallowed by the caller's retry
      // loop — so a missing banner is a silent, misattributed failure of the whole work-lane
      // battery, not a missing log line.
      console.log(CLARA_WORK_BUNDLE_V7_BANNER);
    ...
    const leader = startLeaderLoop(
      {
        ...
        // #945: the payroll_facts lane rides its own workflow (payrollFacts_v1), resolved through
        // the registry `workflows` object exactly like every other facts lane (freeze-lint
        // enqueue-provenance law — a direct workflow-file import handed to start() fails CI).
        // Without this dep, reconciler-documents.mjs's enqueueForLane returns undefined for
