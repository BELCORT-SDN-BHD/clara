...
export const isDash = (s) => DASH_ONLY.test(String(s ?? "").trim());

/** Does this token pass the accept grammar? */
export const isStrictAmount = (s) => AMOUNT_STRICT.test(String(s ?? "").trim());
 ...
 * and ASCII whitespace only. Returns null for anything the DB would also refuse.
 *
...
export function centsOfRaw(raw) {
  const t = String(raw ?? "").trim().toUpperCase().replace(/MYR|RM/g, "").replace(/,/g, "").replace(ASCII_SPACE, "");
  if (!t) return null;
  let negative = false;
  let v = t;
