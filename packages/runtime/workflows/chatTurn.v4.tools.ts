// @frozen
//
// FROZEN — the chatTurn_v4 tool set + the draft_journal_entry write-floor wrapper
...
import { refusalFromDbError, sessionUnboundRefusal } from "./chatTurn.v4.errors.js";
import { readScoped, writeScoped, safeRead, type PgExec, type ToolCtx } from "./chatTurn.v4.infra.js";
    ...
    const receipt = await writeScoped(ctx, async (c: PgExec) => {
      const r = await c.query(
        ...
        [
          ...
          opKey,
          server.booksVersion,
          JSON.stringify(input.counterparty),
 ...
 */
export function buildToolsV4(ctx: ToolCtx) {
  const clientId = ctx.clientId;
  const firmScoped = {
  ...
  return {
    ...
    [DRAFT_TOOL]: tool({
      description:
        ...
        "or a sales credit note (the exact mirror). This is a proposal, not a posting: it produces a review card a bookkeeper approves. " +
        "Provide coding_kind, lines, document_id, counterparty, and an evidence array.",
