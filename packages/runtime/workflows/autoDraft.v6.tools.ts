// @frozen
//
// FROZEN — the autoDraft_v6 tool set + the draft_journal_entry write-floor wrapper (§7-A
// wave-7a-contract.md §3 PR-RUNTIME; skeleton §2a). Split from the impl to hold the file cap.
// Because the sweep task is ALWAYS bound to one client + one document (the admission row),
// every read is client-pinned and the write codes THAT document. The model supplies only
// business content; the wrapper fetches sha256 / resolution / books_version / Tier-A facts
// SERVER-side and stamps the coding_kind marker + the stable op_key. The DB is the
// authoritative Tier-A + entry-shape enforcer; the wrapper's checks are a friendly early
// backstop. There is NO clarify tool — the sweep is unattended.
//
// v6 vs v5 (§7-A, skeleton §2a) — the unattended sweep stops being purchase-only:
//
//   1. `wake_draft_entry`'s arg 14 (the coding_kind marker) becomes `input.coding_kind`
//      instead of the v5 hardcoded literal `"supplier_bill"` (v5.tools.ts:174 — the exact
//      §0.1 headline defect this wave fixes).
//   2. THE COUNTERPARTY CONTRACT, layer 2 of 3 (layer 1 is the schema's superRefine in
//      autoDraft.v6.prompt.ts; layer 3, the only AUTHORITY layer, is the DB draft writer):
//      `deriveCounterpartyKind` computes the counterparty kind FROM coding_kind
//      (vendor for supplier_bill, customer for sales_invoice/sales_credit_note) and the
//      wrapper ALWAYS writes that derived value into the JSON payload sent to the DB —
//      never the model's own optional `kind`, even when it happens to agree. The model
//      never chooses the kind independently; this tool is the derivation authority on the
//      runtime side.
