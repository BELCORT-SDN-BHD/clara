// @frozen
//
// payrollFacts_v1 — THE DATABASE READS AND THE SETTLE VERBS. #945.
//
// ITS OWN MODULE, NOT AN IMPORT OF witnessFacts.v1.dispatch.mjs, for the ticket's own reason: a
// versioned workflow may not couple its shape to another family's frozen files. That module is
// frozen, and a payroll fix that needed one more read there would be an edit to the INVOICE
// lane's frozen closure. The SQL below is deliberately close in shape to its invoice sibling —
// same idioms, same reasons, restated where the reason is load-bearing — and points at this
// family's own doors.
//
...
// `clara.persist_payroll_facts` resolves a citation through the SAME ordering. Its name says
// "witness" because it was minted for the witness pair; it is a NUMBERING, not a questionnaire,
// and inventing a payroll-specific one would guarantee the two drift apart the first time either
// changed. Stated here so a reader does not mistake the reuse for an oversight.

/** The typed egress purpose this lane runs under. The SAME purpose the invoice and statement
 *  witness pairs hold, and migration 0296 §E records why: it is the consent to send a client's
 *  document bytes to a model in order to READ them, which is exactly what this lane does. */
export const PAYROLL_PURPOSE = "witness_extraction";

/** The dispatch event type this lane records against an authorization. */
export const PAYROLL_EVENT_TYPE = "payroll.extraction";

/** A pre-egress size cap on the vision channel. The same 30MB the witness pair holds. */
export const PAYROLL_MAX_VISION_BYTES = 30_000_000;
...
export const PAYROLL_WAIT_BUDGET_MS = 45 * 60 * 1000;

/** The code a spent wait settles under — admitted by clara.fail_payroll_facts' vocabulary. */
export const PAYROLL_WAIT_EXHAUSTED = "wait_exhausted";
 ...
 *  for #945 is DATABASE-first, so in practice this image meets a database that already carries
...
export async function hasPayrollSurface(client) {
  const r = await client.query(
    ...
    + " and to_regprocedure('clara.persist_payroll_facts(uuid,jsonb,jsonb,int)') is not null"
...
export async function preparePayrollDispatch(client, { firmId, clientId, eventSeq, documentSha256 }) {
  const r = await client.query("select clara.prepare_egress_dispatch($1,$2,$3,$4,$5,$6) as v", [
    firmId, clientId, PAYROLL_PURPOSE, eventSeq, PAYROLL_EVENT_TYPE, documentSha256,
  ]);
 ...
 *  spent on document B. */
export async function consumePayrollDispatch(client, { firmId, authorizationId, clientId, eventSeq, documentSha256 }) {
  await client.query("begin");
  let r;
  try {
    r = await client.query("select clara.consume_egress_dispatch($1,$2,$3,$4,$5,$6,$7) as v", [
      firmId, authorizationId, clientId, PAYROLL_PURPOSE, eventSeq, PAYROLL_EVENT_TYPE, documentSha256,
    ]);
...
/** The task's own `version_n` and `engine_id` (both DB-owned: clara.persist_payroll_facts reads
...
export async function readPayrollContext(client, taskId, doc) {
...
export async function waitBudgetExhausted(client, taskId, budgetMs = PAYROLL_WAIT_BUDGET_MS) {
  const r = await client.query(
 ...
 *  clara.persist_payroll_facts resolves it through the same ordering. */
...
/** Settle a RUNNING payroll task terminally through the lane's own verb. A database below the
 *  #945 frontier has no such verb; that is a deploy-order window, not a fault, and it is logged
 *  rather than thrown so the caller's own error stays the one that surfaces. */
export async function settlePayrollFailure(withRuntime, taskId, code, log = console.error) {
  try {
    await callWriter(withRuntime, "select clara.fail_payroll_facts($1,$2) as receipt", [taskId, code]);
  } catch (err) {
    log(`[payroll] could not settle task ${taskId} as '${code}': ${err instanceof Error ? err.message : String(err)}`);
  }
}
