// @frozen
//
// FROZEN — the chatTurn_v9 tool set + the draft_journal_entry write-floor wrapper
...
// text — DRAFT_TOOL's own `.description` string below, `runDraftJournalEntry`, and
// every read tool are byte-unchanged logic (import paths repointed only).
...
import { refusalFromDbError, sessionUnboundRefusal } from "./chatTurn.v9.errors.js";
import { readScoped, writeScoped, safeRead, type PgExec, type ToolCtx } from "./chatTurn.v9.infra.js";
 ...
 */
export function buildToolsV9(ctx: ToolCtx) {
  const clientId = ctx.clientId;
  const firmScoped = {
