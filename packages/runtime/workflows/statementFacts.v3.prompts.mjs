// @frozen
//
// statementFacts_v3 — THE PROMPT CLOSURE for the `statement_facts` WITNESS PAIR. Copied from
// statementFacts.v2.prompts.mjs (F-A1 PR-4 — `docs/plan/active/f-a1-witness-pair-design.md`
// §3.7) and edited in exactly three places; v2 stays exported and frozen. ONE document, TWO
// independent reads through TWO CHANNELS of the SAME provider — text (numbered OCR regions)
// and vision (the original filed bytes) — reapplied to the bank-statement header + line
// skeleton instead of the invoice belt.
//
// WHAT v3 CHANGES, AND WHY (H-02 · H-03). All three are prompt-side halves of a posture whose
// deterministic half lives in `statementFacts.v3.header.mjs`; read that file's header for the
// full reasoning.
//
//   (1) THE PERIOD CONTRADICTION IS GONE (H-02). v2's `period_start` describe invited the model
//       to answer "as the statement's own printed month implies" while SHARED_RULES 1 said
//       "NEVER INFER, NEVER COMPUTE" and rule 3 said "NEVER GUESS ... answer it null". The
//       system rules dominate a per-field describe, so a Maybank corporate statement that
//       prints only `TARIKH PENYATA / STATEMENT DATE : 30/06/25` was answered null/null and
//       refused `header_unreadable` — and nothing on the payload could have said WHY a bound
//       was inferred if it had been. v3 removes the invitation, adds SHARED_RULES 7 telling the
//       reader plainly to answer both bounds null when no band is printed, and moves the
//       derivation to a deterministic server-side transform that records its own basis.
//
//   (2) THE INSTITUTION DESCRIBE STOPS PRETENDING A NAME IS A CODE (H-03). v2 asked for "the
//       bank's short code or name as printed" and relayed whatever came back verbatim into a
//       column the DB binds against `clara.bank_institutions.code`, whose PK check is
//       `^[A-Z0-9]{2,10}$` — so a printed "ALLIANCE BANK" could never be anything but
//       `header_unreadable`. The wire field is UNCHANGED (adding one the DB does not read would
//       be exactly the "answer vocabulary the DB does not verify" mistake this header warns
//       against, and `_stmt_header_norm` would drop it anyway); what changes is that the reader
//       is now told to print the name EXACTLY as it appears and that a server resolves it to
//       the roster code, so a truncated or tidied name is no longer a silent improvement.
//
//   (3) THE HASH NAMES v3. `statementWitnessPromptHash` folds the class string, so every stored
//       read is identifiable as v3-prompted rather than v2-prompted. That is the whole point of
//       the hash and it must move with the version.
//
// FROZEN BY THE SAME DECISION AS witnessFacts.v1.prompts.mjs (design M8): this file sits inside
// statementFacts.v3's frozen import closure, so a prompt edit IS a workflow-body edit and ships
// as statementFacts.v4 + a ceremony. The corpus-tuning loop runs BEFORE the first freeze.
//
...
// families can never be forced to move together by an edit neither author intended. It does not
// import statementFacts.v2.prompts.mjs either: every export below is EDITED by this version, so
// an import would be a re-export of text this file contradicts.
//
...
// here is a plain, directly-typed, NULLABLE value: null means "I could not read this",
 ...
 *  (0038:1175-1273). UNCHANGED from v2: the two H-02/H-03 fixes are a describe rewrite and a
 *  server-side transform, never a new wire field. Order matches the migration's own
...
const headerShape = z.object({
  institution_code: z.string().nullable().describe("the bank's identity as the statement itself gives it: its short code if the statement prints one (e.g. 'MBB'), otherwise the institution's full name EXACTLY as printed on the letterhead (e.g. 'ALLIANCE BANK', 'Malayan Banking Berhad') — a server matches a printed name against the bank roster, so report it whole and unabbreviated rather than tidying it"),
  ...
  period_start: z.string().nullable().describe("ISO YYYY-MM-DD — the first day of the statement period AS PRINTED in the statement's own from–to band; null if the statement prints no such band (see rule 7 — never derive one)"),
  period_end: z.string().nullable().describe("ISO YYYY-MM-DD — the last day of the statement period AS PRINTED in the statement's own from–to band; null if the statement prints no such band (see rule 7 — never derive one)"),
  statement_date: z.string().nullable().describe("ISO YYYY-MM-DD — the statement's own printed date (a Malaysian statement often labels it 'TARIKH PENYATA' / 'STATEMENT DATE')"),
...
const SHARED_RULES = [
  ...
  "7. THE PERIOD BAND IS RULE 1 APPLIED TO DATES, AND IT IS WORTH ITS OWN LINE. Answer",
  "   period_start and period_end ONLY from a from–to band the statement actually prints",
  "   ('01/06/2025 - 30/06/2025', 'FOR THE PERIOD ...'). Many Malaysian statements print no band",
  "   at all and only a statement date ('TARIKH PENYATA / STATEMENT DATE : 30/06/25'). When that",
  "   is what you see, answer BOTH bounds null and answer statement_date — a deterministic",
  "   server derives the covered month from that date and records on the receipt that it derived",
  "   it. A month you work out yourself would be indistinguishable from one the bank printed,",
  "   and the whole point of this pass is that a reader of the result can tell the two apart.",
];
...
export function statementWitnessPromptHash(channel) {
  ...
  return createHash("sha256")
    .update(JSON.stringify(["statementFacts.v3", channel, system, vocabulary]), "utf8")
    .digest("hex");
}
...
//
// THE TWO v3 TRANSFORMS DO NOT LIVE HERE. `normalizeStatementHeaderV3` (institution roster
// resolution, period basis) is applied by the PERSIST payload builder in
// statementFacts.v3.behavior.mjs, AFTER this relay and in one place — so a channel's stored
// read and the receipt describing it are built from the same object, and so this function stays
