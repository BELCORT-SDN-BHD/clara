// @frozen
//
// FROZEN — part of the autoDraft_v10 closure. A NEW frozen closure beside the byte-untouched
// autoDraft_v1..v9 (ARCHITECTURE Appendix A: a behavioural change ships as a new _vN export,
// never an in-place edit — the registry repoints `autoDraft:` here).
//
// ================================ WHAT v10 IS, IN ONE PLACE =================================
//
// H-17, the beta walk's row: the unattended coder refused itself on a sales invoice and opened a
// human question no answer could settle. ONE function moves — `refusalFromDbError`'s native
// 23505 arm — from a SUBSTRING test on the constraint name to an EXACT, closed constraint-name
// map with a reason token per arm and an `internal` fall-through on an unknown name. The
// argument, the four proven index names and the reason vocabulary live in
// autoDraft.v10.uniques.ts's header; the arm itself is in autoDraft.v10.errors.ts.
//
// THIS FILE IS A VERSION-RENAME of autoDraft.v9.tools.ts, byte-identical but for ONE import
// line: `./autoDraft.v9.errors.js` -> `./autoDraft.v10.errors.js` (the PR body quotes the diff).
// v10 REUSES v9's .prompt, .post, .settle, .infra, .usage and .postcall unchanged — none calls
// `refusalFromDbError`, and copying a frozen body byte-for-byte buys no behaviour.
//
// THE SALES-SIDE KIND WAS ALREADY PASSED, BEFORE v10 — see `deriveCounterpartyKind` below and
// its call site. H-17's "the sales-side pre-flight must pass kind=customer" half is a NO-OP:
// already true in v9, unchanged here. What closes the class is this PR's DB half.
//
// (v9 was F-A1 PR-3a: it widened the coding-lane toolfaces to the witness-pair regime; design
// docs/plan/active/f-a1-witness-pair-design.md §3.8, the M7 selection rule; Annex B row M7.)
//
// THE v7->v9 FINDING, ONCE (carried, not re-argued): F-A1 PR-1 shipped the witness-pair
// extraction regime beside legacy `invoice_facts` and repointed the DB resolver to prefer the
// newest generation ACROSS regimes by `extracted_at`, never by the per-lane `version_n`. v7's
// `readInvoiceFactState` knew none of it — it filtered to `invoice_facts` alone, picked a
// generation by a cross-kind `Math.max(version_n)`, and carried a stale `engine_confidence >=
// 0.95` mirror that would zero out every witness document's hint (a witness region carries
// `engine_confidence = NULL` by design, §3.4). Full argument: autoDraft.v9.tools.ts's header.
//
...
// THE TOOL SET ITSELF LIVES IN autoDraft.v10.toolset.ts — split out at v9 (ceiling), seam
// "wrappers vs wiring": above are pure/DB-facing WRAPPERS a unit test calls directly; the
// toolset module is the AI-SDK wiring that binds them to a ctx.
