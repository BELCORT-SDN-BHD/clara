// @frozen
//
// FROZEN — the claraWork_v6 workflow entry: the CUT PHASE's Work successor (wave 2026-09-25). A
// REPOINT of the `claraWork` registry key, never an edit: v1's … v5's file sets are byte-untouched
...
//
// WHAT CHANGED FROM v5, AND IT IS ONE THING: #1030's CONFIRMATION, ASKED BEFORE ANYTHING ELSE.
//
// #885 (migration 0268) retires every Work parked on a question about a document fact somebody
// corrects, and admits NOTHING in its place — because re-admission ON THE CORRECTED FACTS needs
// somebody to re-read the corrected document and propose a basis from it, and that is an
// interpretation act no SQL door can perform. The runtime now does it: a belt re-derives the basis
// from the document's LIVE facts and admits a successor through the ordinary admission door.
//
// THIS BODY IS THE OTHER HALF OF THAT CONTRACT — the half that makes it safe. Before a v6 run
// reads knowledge, spends a token or reaches a model, it asks whether this Work is such a
// successor, and if it is, it PARKS on a question that names BOTH figures: what the retired Work
// was admitted on, and what the document says now. Nothing may post until a person answers it.
// That is the ruling's own wording, and the park is how a body enforces it rather than promising
// it: there is no path from admission to a posting tool that does not pass through this hook.
//
// WHY THE PROBE IS TERMINAL WHEN IT FAILS, and it is the one place v6 is stricter than v5. Every
// other diagnostic in this closure is wrapped, because a diagnostic may not decide an accounting
// outcome. This is not a diagnostic: it decides whether a human confirms a figure before it can
// be posted, so a read that did not land settles the run `failed`/`internal` — RECOVERABLE,
// NOTHING POSTED — rather than letting it proceed as though the answer had been "no".
//
// THE DEPLOY ORDER FOLLOWS FROM THAT, IN ONE DIRECTION. MIGRATION 0321 MUST BE LIVE BEFORE THIS
// IMAGE RUNS ANY WORK, on top of v5's 0230, v4's 0192/0216 and v3's 0195. Against a database
// without 0321 the probe cannot answer and EVERY Work stops, refusing to post rather than posting
// a figure nobody confirmed. Contained, corrupting nothing, and loud — a whole lane that refuses
// is a page anybody notices, which is the correct failure for a deploy-order mistake. The REVERSE
// order is FREE: 0321 against a v5 image adds doors that nothing calls.
//
...
// recheck, the #639 particulars pair in full, the question finder, and "success is decided by a
// receipt, never by a stream ending" (ARCHITECTURE §6). The TOOL ROSTER IS v5's, unchanged: v6
// adds no tool and widens no schema. What moves in the bundle is the instructions text (one
// stanza, `SOURCE_CORRECTION_SUCCESSOR_STANZA`) and this closure's own ids, so the digest moves
// and `tests/pinned-work-bundle.mjs` can tell a v6 run from a v5 one on the record rather than on
// the calendar.
//
// THE BUDGETS DO NOT MOVE, and the addition does not press on them: the confirmation park happens
// BEFORE the loop and spends no segment, no model call and no tool call.
//
// ROLLBACK TO v5 stops asking the confirmation and changes no database state. It is NOT free while
// the belt is live: a successor the belt admits under a v5 image would run WITHOUT the
// confirmation — v5 has no arm for it — and could post a re-derived basis nobody was shown. A
// rollback therefore disables the belt first (the image that carries it is the one being rolled
// back, so this is automatic when the whole image goes) and drains parked confirmations rather
// than assuming they resume identically.
...
import { CLARA_WORK_BUDGETS_V6 } from "./claraWork.v6.bundle.js";
import {
...
export async function claraWork_v6(input: { taskId: string }): Promise<{ taskId: string; outcome: string; segments: number }> {
  "use workflow";
  ...
  const settle = async (
    ...
    settled = true;
    outcome = o;
    await settleWorkStepV6(taskId, runId, SETTLE_TRACE_SEQ, { outcome: o, errorCode, error, result });
  };

  try {
    const claim = await claimWorkRunStepV6(taskId);
    ...
    for (; segment < budgets.segments; segment++) {
      const seg = await runWorkSegmentStepV6(taskId, work, runId, segment, messages, knowledge.knowledge_version);
        ...
        if (pending) {
          ...
          if (opened === null) {
            ...
            await settle(
              ...
              null,
              null,
              completedResultV6(seg.posted, confirm.confirmed, { toolCalls, replans, transientRetries }, segment + 1, tokens, particulars, knowledge),
            );
            break;
          }
