// @frozen
//
// FROZEN — the durable "use step" bodies of chatTurn_v2 (Slice-6 coding floor).
...
// chatTurn.v2.infra.ts / chatTurn.v2.tools.ts (imported RELATIVELY so the freeze-lint
...
import { pools, resolveModel, readScoped } from "./chatTurn.v2.infra.js";
import { buildToolsV2 } from "./chatTurn.v2.tools.js";

export { SYSTEM_PROMPT_V2 };
...
function messageFromParts_v2(role: string, parts: ClaraPart[]): ModelMessage | null {
  const text = parts
    .map((p) => {
...
export async function loadTaskStepV2(
  taskId: string,
  ...
  return pools().withRuntime(async (c) => {
    ...
    if (r.rowCount === 0) throw new Error(`chatTurn_v2: task ${taskId} not found`);
    const row = r.rows[0]!;
    return {
...
export async function loadContextStepV2(
  sessionId: string,
  ...
  const history = await pools().withRuntime(async (c) => {
    ...
    return r.rows
      .map((row) => messageFromParts_v2(String(row.role), (row.parts ?? []) as ClaraPart[]))
...
export async function runModelSegmentStepV2(
  taskId: string,
  model: string,
  clientId: string | null,
  firmId: string,
  ...
  const result = streamText({
    ...
    system: systemExtra ? `${SYSTEM_PROMPT_V2}\n\n${systemExtra}` : SYSTEM_PROMPT_V2,
    messages,
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    tools: tools as any,
