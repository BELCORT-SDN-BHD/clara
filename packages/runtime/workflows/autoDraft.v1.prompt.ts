// @frozen
//
// FROZEN — the prompt text + draft input schema + typed-part shapes of autoDraft_v1
// (Wave A auto-draft sweep; contract §3 / companion §4-5). A NEW frozen closure beside
// the byte-untouched chatTurn_v3 (ARCHITECTURE Appendix A: a behavioural change ships as
// a new workflow class, never an in-place edit). The freeze-lint hash-locks this file as
// part of autoDraft.v1.ts's import closure. It imports NO first-party infrastructure — the
// DB-backed tools are BUILT with an injected pool handle inside the step that runs them
// (autoDraft.v1.impl.ts).
//
// autoDraft_v1 is the UNATTENDED coding lane: there is NO human in the loop, so there is
...
//
// Deliberately a v2/v3-independent local copy of the draft schema + part shapes (a versioned
// workflow must never couple its shape to another version's frozen file — the same reason
// chatTurn.v3 keeps its own clarify tool). Third-party imports (ai, zod) are outside the
// freeze surface.
...
export const SYSTEM_PROMPT_AUTODRAFT = [
  ...
  "lines, the document_id, the vendor, and an evidence array.",
  "",
...
// wrapper in autoDraft.v1.tools.ts fetches sha256 / resolution / books_version /
