...
// once any run can be in flight. autoDraft_v1 is a NEW frozen closure (registry adds
// `autoDraft: autoDraft_v1`); chatTurn_v3 and every other export are byte-untouched. Do NOT
// edit this file or its import closure (autoDraft.v1.impl / .tools / .infra / .prompt / .errors)
// once deployed — ship a behavioural change as autoDraft_v2.
//
// autoDraft_v1 — the UNATTENDED auto-draft sweep (contract §3 / companion §4-5). One admitted
...
} from "./autoDraft.v1.impl.js";
import { isQuestionShaped } from "./autoDraft.v1.prompt.js";
import { noDraftRefusal } from "./autoDraft.v1.errors.js";

export async function autoDraft_v1(input: { taskId: string }): Promise<{ taskId: string; outcome: string }> {
  "use workflow";
