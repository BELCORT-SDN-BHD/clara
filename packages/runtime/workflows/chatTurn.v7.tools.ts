// @frozen
//
// FROZEN — the chatTurn_v7 tool set + the draft_journal_entry write-floor wrapper
...
//
// v7 vs v6 (Wave B, AMB-1/FORK-6): the draft-wrapper's internal server-side re-fetch
// (runDraftJournalEntry, below) KEEPS purpose "coding" and does NOT set the GUC — it
// only needs books_version, so it stays wiki-dark by design (AMB-1). The
// get_context_pack TOOL exposed to the model is the one call site here that feeds
// MODEL context: its `purpose` input is pinned to the literal enum "wiki_coding"
// (z.literal — a model-supplied free string can no longer reach the DB as a
// different purpose, hardening WB-R6(2)'s structural claim per FORK-6(b)), and its
// execute sets the txn-local `clara.pack_consumer = 'v25'` GUC immediately before the
// pack fetch, on the SAME client inside the SAME readScoped transaction.
...
import { refusalFromDbError, sessionUnboundRefusal } from "./chatTurn.v7.errors.js";
import { readScoped, writeScoped, safeRead, type PgExec, type ToolCtx } from "./chatTurn.v7.infra.js";
 ...
 */
export function buildToolsV7(ctx: ToolCtx) {
  const clientId = ctx.clientId;
  const firmScoped = {
