// @frozen
//
// FROZEN — the prompt text + draft input schema + typed-part shapes of autoDraft_v3
...
// freeze-lint hash-locks this file as part of autoDraft.v3.ts's import closure. It imports
// NO first-party infrastructure — the DB-backed tools are BUILT with an injected pool
// handle inside the step that runs them (autoDraft.v3.impl.ts).
//
...
//
// autoDraft_v3 is the UNATTENDED coding lane: there is NO human in the loop, so there is
...
export const SYSTEM_PROMPT_AUTODRAFT_V3 = [
...
// wrapper in autoDraft.v3.tools.ts fetches sha256 / resolution / books_version /
