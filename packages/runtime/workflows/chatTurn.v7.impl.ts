// @frozen
//
// FROZEN — the durable "use step" bodies of chatTurn_v7 (Slice-6 coding floor).
...
// chatTurn.v7.infra.ts / chatTurn.v7.tools.ts (imported RELATIVELY so the freeze-lint
...
//
// v7 vs v6 (Wave B, AMB-1/FORK-6): loadContextStepV7's context-pack fetch — the ONLY
// get_context_pack call site in this file that feeds MODEL context — switches its
// purpose literal from "chat" to "wiki_coding" and sets the txn-local
// `clara.pack_consumer = 'v25'` GUC immediately before the fetch, in the SAME
// wake-scoped transaction (readScoped already opens one txn per call — see
// lib/pools.mjs's withReadWakeScoped, which begin/rollback-wraps the whole callback).
// Everything else in this file is an unmodified version-rename of v6.
...
import {
  ...
  type AiContentPart,
  type JeReviewPart,
} from "./chatTurn.v7.prompt.js";
import { pools, resolveModel, readScoped } from "./chatTurn.v7.infra.js";
import { buildToolsV7 } from "./chatTurn.v7.tools.js";

export { SYSTEM_PROMPT_V7 };
...
function messageFromParts_v7(role: string, parts: ClaraPart[]): ModelMessage | null {
  const text = parts
    .map((p) => {
...
export async function loadTaskStepV7(
  taskId: string,
  ...
  return pools().withRuntime(async (c) => {
    ...
    if (r.rowCount === 0) throw new Error(`chatTurn_v7: task ${taskId} not found`);
    const row = r.rows[0]!;
    return {
...
export async function loadContextStepV7(
  sessionId: string,
  ...
  const history = await pools().withRuntime(async (c) => {
    ...
    return r.rows
      .map((row) => messageFromParts_v7(String(row.role), (row.parts ?? []) as ClaraPart[]))
...
export async function runModelSegmentStepV7(
  taskId: string,
  model: string,
  clientId: string | null,
  firmId: string,
  ...
  const tools = buildToolsV7({ firmId, clientId, createdBy, taskId });
  const result = streamText({
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    model: resolveModel(model) as any,
    system: systemExtra ? `${SYSTEM_PROMPT_V7}\n\n${systemExtra}` : SYSTEM_PROMPT_V7,
    messages,
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    tools: tools as any,
