// @frozen
//
// FROZEN — the chatTurn_v8 tool set + the draft_journal_entry write-floor wrapper
...
// friendly early backstop.
//
// v8 vs v7 (owner-approved closing batch, 2026-07-29): the ONLY change in THIS file
// is the DRAFT_TOOL's own `.description` string below, echoing chatTurn.v8.prompt.ts's
// two rule changes (#46b the SST-zero tax-rule propagation, #35 bind-existing
// counterparty) at the tool-call surface. `runDraftJournalEntry` itself is
// UNCHANGED — it already forwards `input.counterparty` verbatim, unconditionally,
// to `wake_draft_entry` (line "input.counterparty ? JSON.stringify(input.counterparty) : null"
// below), so an `{existing_id}` proposal was already fully wired end-to-end; what
// was missing was model-facing prompt/schema guidance (chatTurn.v8.prompt.ts), not
// wrapper logic. The DB-side reconciliation walls are untouched and stay the
// enforcement. Every read tool, and the wrapper's own server-side logic, is an
...
import { refusalFromDbError, sessionUnboundRefusal } from "./chatTurn.v8.errors.js";
import { readScoped, writeScoped, safeRead, type PgExec, type ToolCtx } from "./chatTurn.v8.infra.js";
 ...
 */
export function buildToolsV8(ctx: ToolCtx) {
  const clientId = ctx.clientId;
  const firmScoped = {
