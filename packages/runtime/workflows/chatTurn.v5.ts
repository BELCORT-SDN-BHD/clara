...
// body is IMMUTABLE once any run can be in flight. chatTurn_v5 is a NEW frozen closure
// beside the byte-untouched chatTurn_v4/v3/v1 (the registry repoints `chatTurn:` here;
// prior exports stay until zero non-terminal runs of those versions remain). Do NOT
// edit this file or its import closure (chatTurn.v5.impl / .tools / .infra / .prompt /
// .errors) once deployed.
//
// chatTurn_v5 — the durable chat turn WITH the Slice-6 coding floor (contract §3),
// the Wave-A2 sales direction (v4), AND the generic journal_entry lane (the §9
// Gate-B live find: no chat version could send a NULL coding_kind, so voucher-style
// entries the DB has always accepted were undraftable via chat). Like v1 (load
...
// firm-scoped read tools, the draft_journal_entry write tool (v5: coding_kind
...
import {
  ...
  settleStep,
  closeStreamStep,
} from "./chatTurn.v5.impl.js";
import { CLARIFY_FRAMING, type ClaraPart } from "./chatTurn.v5.prompt.js";
import { codingIncompleteRefusal } from "./chatTurn.v5.errors.js";
...
export async function chatTurn_v5(input: { taskId: string }): Promise<{ taskId: string; outcome: string; segments: number }> {
  "use workflow";
  ...
  try {
    ...
    const task = await loadTaskStepV5(taskId);
    const ctx = await loadContextStepV5(task.sessionId, task.clientId, task.firmId, task.createdBy);
    ...
    for (; segment < MAX_SEGMENTS; segment++) {
      const seg = await runModelSegmentStepV5(taskId, task.model, task.clientId, task.firmId, task.createdBy, messages, systemExtra);
