// @frozen
//
// FROZEN — the durable "use step" bodies of chatTurn_v5 (Slice-6 coding floor).
...
// chatTurn.v5.infra.ts / chatTurn.v5.tools.ts (imported RELATIVELY so the freeze-lint
...
import {
  ...
  type AiContentPart,
  type JeReviewPart,
} from "./chatTurn.v5.prompt.js";
import { pools, resolveModel, readScoped } from "./chatTurn.v5.infra.js";
import { buildToolsV5 } from "./chatTurn.v5.tools.js";

export { SYSTEM_PROMPT_V5 };
...
function messageFromParts_v5(role: string, parts: ClaraPart[]): ModelMessage | null {
  const text = parts
    .map((p) => {
...
export async function loadTaskStepV5(
  taskId: string,
  ...
  return pools().withRuntime(async (c) => {
    ...
    if (r.rowCount === 0) throw new Error(`chatTurn_v5: task ${taskId} not found`);
    const row = r.rows[0]!;
    return {
...
export async function loadContextStepV5(
  sessionId: string,
  ...
  const history = await pools().withRuntime(async (c) => {
    ...
    return r.rows
      .map((row) => messageFromParts_v5(String(row.role), (row.parts ?? []) as ClaraPart[]))
...
export async function runModelSegmentStepV5(
  taskId: string,
  model: string,
  clientId: string | null,
  firmId: string,
  ...
  const tools = buildToolsV5({ firmId, clientId, createdBy, taskId });
  const result = streamText({
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    model: resolveModel(model) as any,
    system: systemExtra ? `${SYSTEM_PROMPT_V5}\n\n${systemExtra}` : SYSTEM_PROMPT_V5,
    messages,
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    tools: tools as any,
