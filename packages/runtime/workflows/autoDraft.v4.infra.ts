// @frozen
//
// FROZEN — injected-infrastructure accessors + wake-credential scoping for the autoDraft_v4
// closure. Split out of the impl so the file-cap discipline holds. Infrastructure (pools,
...
// model-id fix lives in the DB default (migration 0033) and in autoDraft.v4.impl.ts's own
// runAutoDraftModelStep, which surfaces the stream's real error instead of trusting ai@7's
// generic NoOutputGeneratedError fallback — resolveModel itself is unchanged: it still just
// forwards whatever model_snapshot string it is handed to @ai-sdk/openai's default provider.

import { openai } from "@ai-sdk/openai";
import { readToolRefusalMessage } from "./autoDraft.v4.errors.js";
