// @frozen
//
// FROZEN — the durable "use step" bodies of chatTurn_v9 (§7-A skeleton §2f). Every
// step's DB effects are workflow semantics and therefore frozen (ship a change as a
// new _vN). Infrastructure + the tool set live in the sibling frozen modules
// chatTurn.v9.infra.ts / chatTurn.v9.tools.ts (imported RELATIVELY so the
// freeze-lint follows + hash-locks them); this file holds only the steps + the
// attachment-aware transcript folder. Per-attempt credentials are minted inside
// the step that uses them and never cross a step boundary (§4.1).
//
// v9 vs v8 (§7-A, skeleton §2f): this file is an unmodified version-rename of v8.
// This wave's ONE behavioural change (the sales-vs-supplier-bill framing
// reinforcement) is confined entirely to chatTurn.v9.prompt.ts's system prompt
// text — every step body here (claim/load/model-segment/hook/checkpoint/settle/
// close, and consumeChatTurnModelResult's stream-error tagging) is byte-unchanged
// logic (import paths + the V8->V9 identifier suffix repointed only).
...
import {
  ...
  type AiContentPart,
  type JeReviewPart,
} from "./chatTurn.v9.prompt.js";
import { pools, resolveModel, readScoped } from "./chatTurn.v9.infra.js";
import { buildToolsV9 } from "./chatTurn.v9.tools.js";

export { SYSTEM_PROMPT_V9 };
...
function messageFromParts_v9(role: string, parts: ClaraPart[]): ModelMessage | null {
  const text = parts
    .map((p) => {
...
export async function loadTaskStepV9(
  taskId: string,
  ...
  return pools().withRuntime(async (c) => {
    ...
    if (r.rowCount === 0) throw new Error(`chatTurn_v9: task ${taskId} not found`);
    const row = r.rows[0]!;
    return {
...
export async function loadContextStepV9(
  sessionId: string,
  ...
  const history = await pools().withRuntime(async (c) => {
    ...
    return r.rows
      .map((row) => messageFromParts_v9(String(row.role), (row.parts ?? []) as ClaraPart[]))
 ...
 *  chatTurn.v9.ts's top-level catch, which only ever sees the reconstructed
 *  FatalError, not this original object. errorCodeFromCaughtError (chatTurn.v9.ts)
...
export async function consumeChatTurnModelResult(
  ...
  try {
    ...
    if (streamError != null) {
      ...
      // load-bearing for what chatTurn.v9.ts eventually sees.
...
export async function runModelSegmentStepV9(
  taskId: string,
  model: string,
  clientId: string | null,
  firmId: string,
  ...
  const tools = buildToolsV9({ firmId, clientId, createdBy, taskId });
  const result = streamText({
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    model: resolveModel(model) as any,
    system: systemExtra ? `${SYSTEM_PROMPT_V9}\n\n${systemExtra}` : SYSTEM_PROMPT_V9,
    messages,
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    tools: tools as any,
