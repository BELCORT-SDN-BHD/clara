...
export const BANK_GUIDANCE = [
  ...
  "",
  "FOUR OF THESE VERBS POST TO THE BOOKS (settle_from_bank_line, resolve_and_book_bank_line, and",
  "any other tool whose description says it posts) — treat them with the same care as",
  "post_journal_entry: only when the human has asked for the underlying line or exception to be",
  "resolved that way. The rest (matching, unmatching, reconciling, registering an account,",
  "proposing an exception or an identifier promotion) are ordinary bank-lane actions a bookkeeper",
  "does routinely — you may take them when they plainly follow from what the human asked, the",
  "same judgement you already apply to reads. When genuinely unsure, ask.",
].join("\n");
