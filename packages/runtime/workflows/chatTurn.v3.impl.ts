// @frozen
//
// FROZEN — the durable "use step" bodies of chatTurn_v3 (Slice-6 coding floor).
...
// chatTurn.v3.infra.ts / chatTurn.v3.tools.ts (imported RELATIVELY so the freeze-lint
...
import {
  ...
  type AiContentPart,
  type JeReviewPart,
} from "./chatTurn.v3.prompt.js";
import { pools, resolveModel, readScoped } from "./chatTurn.v3.infra.js";
import { buildToolsV2 } from "./chatTurn.v3.tools.js";

export { SYSTEM_PROMPT_V3 };
...
function messageFromParts_v3(role: string, parts: ClaraPart[]): ModelMessage | null {
  const text = parts
    .map((p) => {
...
export async function loadTaskStepV3(
  taskId: string,
  ...
  return pools().withRuntime(async (c) => {
    ...
    if (r.rowCount === 0) throw new Error(`chatTurn_v3: task ${taskId} not found`);
    const row = r.rows[0]!;
    return {
...
export async function loadContextStepV3(
  sessionId: string,
  ...
  const history = await pools().withRuntime(async (c) => {
    ...
    return r.rows
      .map((row) => messageFromParts_v3(String(row.role), (row.parts ?? []) as ClaraPart[]))
...
export async function runModelSegmentStepV3(
  taskId: string,
  model: string,
  clientId: string | null,
  firmId: string,
  ...
  const tools = buildToolsV2({ firmId, clientId, createdBy, taskId });
  const result = streamText({
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    model: resolveModel(model) as any,
    system: systemExtra ? `${SYSTEM_PROMPT_V3}\n\n${systemExtra}` : SYSTEM_PROMPT_V3,
    messages,
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    tools: tools as any,
