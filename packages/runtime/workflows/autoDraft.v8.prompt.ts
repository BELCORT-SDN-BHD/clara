...
//
// THIS FILE (prompt) — an UNMODIFIED version-rename of v7: the whole direction/leg-shape/SST/
// watch/wiki/MYR-only system prompt body, the draft_journal_entry input schema (including the
// region_idx evidence shape and its superRefine counterparty contract), and every typed part
// shape (AiContentPart/JeReviewPart/RefusalPart/DraftToolResult/AutoDraftOutcome,
// toAutoDraftOutcome/isDoubleCodedReason/isQuestionShaped) are byte-identical — only the
// SYSTEM_PROMPT_AUTODRAFT_V7 -> _V8 identifier and the version in the prose moved. Nothing about
// a document's regime (legacy vs witness) is visible to the model at this layer: the region
// vocabulary (`idx`, `field_path`) is identical across both, so no prompt sentence needed a word
// changed for F-A1.
//
...
export const SYSTEM_PROMPT_AUTODRAFT_V8 = [
...
// wrapper in autoDraft.v8.tools.ts fetches sha256 / resolution / books_version /
