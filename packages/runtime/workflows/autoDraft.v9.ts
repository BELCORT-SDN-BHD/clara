// @frozen
//
// FROZEN — part of the autoDraft_v9 closure (F-A1 PR-3a: the consumer re-versioning that
// widens the coding-lane toolfaces to the witness-pair regime; see autoDraft.v9.tools.ts for
// the one statement of what changed and why). A NEW frozen closure beside the byte-untouched
...
//
// THIS FILE (the workflow entry) — v9 vs v8: the claim/recover/model/question/close happy path,
// refusalFromCaughtError with its WDK retry-prefix pattern, and the finally block are
// byte-carried. What is NEW is the two POST branches and the settle ORDERING they depend on
// (both documented at the branch itself). Do NOT edit this file or its import closure
// (autoDraft.v9.impl / .tools / .toolset / .post / .postcall / .settle / .infra / .prompt /
// .errors) once deployed — ship a behavioural change as autoDraft_v10.
...
} from "./autoDraft.v9.impl.js";
import { isQuestionShaped } from "./autoDraft.v9.prompt.js";
import { noDraftRefusal } from "./autoDraft.v9.errors.js";
...
export async function autoDraft_v9(input: { taskId: string }): Promise<{ taskId: string; outcome: string }> {
  "use workflow";
