// @frozen
//
// FROZEN — the prompt text + draft input schema + typed-part shapes of autoDraft_v4
// (Wave-A auto-draft sweep; contract §3 / companion §4-5). A NEW frozen closure beside
// the byte-untouched autoDraft_v1/v2/v3 (ARCHITECTURE Appendix A: a behavioural change ships
// as a new _vN export, never an in-place edit; the registry repoints `autoDraft:` here). The
// freeze-lint hash-locks this file as part of autoDraft.v4.ts's import closure. It imports
// NO first-party infrastructure — the DB-backed tools are BUILT with an injected pool
// handle inside the step that runs them (autoDraft.v4.impl.ts).
//
// v4 vs v3 (ledger #44/GitHub #42): this file is an UNMODIFIED version-rename of v3 — the
// prompt text, draft schema, and typed-part shapes are unaffected by this wave's fix. The
// bug this version ships to fix lives entirely OUTSIDE the model-facing surface: the
// one-click admission path's model_snapshot default (packages/db/migrations/0033) and the
// model step's stream-error handling (autoDraft.v4.impl.ts's runAutoDraftModelStep).
//
...
//
// autoDraft_v4 is the UNATTENDED coding lane: there is NO human in the loop, so there is
...
export const SYSTEM_PROMPT_AUTODRAFT_V4 = [
...
// wrapper in autoDraft.v4.tools.ts fetches sha256 / resolution / books_version /
