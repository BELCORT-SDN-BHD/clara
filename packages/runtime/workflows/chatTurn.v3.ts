...
// body is IMMUTABLE once any run can be in flight. chatTurn_v3 is a NEW frozen closure
...
// import closure (chatTurn.v3.impl / .tools / .infra / .prompt / .errors) once deployed.
//
// chatTurn_v3 — the durable chat turn WITH the Slice-6 coding floor (contract §3). Like
...
import {
  ...
  settleStep,
  closeStreamStep,
} from "./chatTurn.v3.impl.js";
import { CLARIFY_FRAMING, type ClaraPart } from "./chatTurn.v3.prompt.js";
import { codingIncompleteRefusal } from "./chatTurn.v3.errors.js";
...
export async function chatTurn_v3(input: { taskId: string }): Promise<{ taskId: string; outcome: string; segments: number }> {
  "use workflow";
  ...
  try {
    ...
    const task = await loadTaskStepV3(taskId);
    const ctx = await loadContextStepV3(task.sessionId, task.clientId, task.firmId, task.createdBy);
    ...
    for (; segment < MAX_SEGMENTS; segment++) {
      const seg = await runModelSegmentStepV3(taskId, task.model, task.clientId, task.firmId, task.createdBy, messages, systemExtra);
