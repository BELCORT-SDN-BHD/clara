// @frozen
//
// FROZEN — the prompt text + tool registry + typed-part shapes of chatTurn_v5
...
// this file as part of chatTurn.v5.ts's import closure.
//
// v5 vs v4 (the delta that makes this a new version — Wave-A2 §9 Gate-B live find):
//   * the GENERIC journal entry. `coding_kind` gains "journal_entry", which the
//     wrapper maps to NULL at the DB writer — the lane the DB has always supported
//     for vouchers/adjustments (no counterparty proposal or coded-shape floor; the
//     balance + control-class rules still bind). `counterparty` becomes optional
//     (required by the DB for the three document kinds; usually absent on a JV
//     unless a control-class line needs one). No chat version could send NULL
//     before (v2/v3 hardcoded supplier_bill; v4's enum had only document kinds).
// v4 vs v3: the SALES direction (coding_kind passthrough, kind-neutral
...
// pool handle inside the step that runs them (chatTurn.v5.impl.ts).
 ...
 *  voucher-style journal entry (v5) — for a HUMAN to approve. She never approves and
...
// chatTurn.v5.impl.ts) fetches sha256 / resolution / books_version / op_key
  ...
  counterparty: z
    ...
    .describe(
      "The counterparty (the supplier on a supplier_bill; the CUSTOMER on a sales entry): an " +
        ...
        "registration number is a rename, not a new counterparty). REQUIRED for the three " +
        "document kinds; omit on a journal_entry unless a control-class line needs one.",
    ),
  evidence: z
    .array(
      z.object({
 ...
 */
export function toTypedParts_v5(content: readonly AiContentPart[]): ClaraPart[] {
  const out: ClaraPart[] = [];
