// @frozen
//
// FROZEN — the durable "use step" bodies of autoDraft_v3 (Wave B — WB-R7, same ceremony
// as chatTurn_v7). Every step's DB effects are workflow semantics and therefore frozen
// (ship a change as a new class/version). Infrastructure + the tool set live in the
// sibling frozen modules autoDraft.v3.infra.ts / autoDraft.v3.tools.ts (imported
// RELATIVELY so the freeze-lint follows + hash-locks them). Per-attempt credentials are
// minted inside the step that uses them and never cross a step boundary (§4.1).
//
// v3 vs v2: this file has NO get_context_pack call site of its own (the sweep does not
// preload a pack — the model fetches it via the get_context_pack TOOL in
// autoDraft.v3.tools.ts, which carries the AMB-1/FORK-6 wiki-purpose + GUC delta). This
// file is therefore an unmodified version-rename of v2.

import { streamText, stepCountIs } from "ai";
import { getWritable, getWorkflowMetadata } from "workflow";
import type { ModelMessage } from "ai";
import {
  SYSTEM_PROMPT_AUTODRAFT_V3,
  DRAFT_TOOL,
  toAutoDraftOutcome,
  type AutoDraftOutcome,
  type AiContentPart,
  type JeReviewPart,
} from "./autoDraft.v3.prompt.js";
import { pools, resolveModel, type ToolCtx } from "./autoDraft.v3.infra.js";
import { buildAutoDraftTools } from "./autoDraft.v3.tools.js";

export { SYSTEM_PROMPT_AUTODRAFT_V3 };
...
export async function runAutoDraftModelStep(
  ...
  const result = streamText({
    ...
    system: SYSTEM_PROMPT_AUTODRAFT_V3,
    messages,
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    tools: tools as any,
