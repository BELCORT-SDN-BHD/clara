// @frozen
//
// FROZEN — the prompt text + tool registry + typed-part shapes of chatTurn_v4
...
// this file as part of chatTurn.v4.ts's import closure.
//
// v4 vs v3 (the delta that makes this a new version — Wave-A2 §9 live find):
//   * the SALES direction. `draft_journal_entry` gains a `coding_kind`
//     (supplier_bill | sales_invoice | sales_credit_note) the wrapper passes to the
//     DB writer (v3 hardcoded 'supplier_bill', so the frozen tool could not draft
//     the AR side the 0015 DB floor already enforces), the input's `vendor` field
//     becomes the kind-neutral `counterparty`, and the prompt teaches the sales/CN
//     shapes, direction resolution, rename-not-duplicate counterparty matching, and
//     the professional-vigilance (edge-case visibility) duty.
...
// pool handle inside the step that runs them (chatTurn.v4.impl.ts).
 ...
 *  entry — a supplier bill OR a sales invoice / sales credit note (v4) — for a HUMAN
 *  to approve. She never approves and never posts a figure unreviewed
 *  (agent-never-signs — ADR-015). */
export const SYSTEM_PROMPT_V4 = [
  ...
  "and you can DRAFT one journal entry — a supplier bill, a sales invoice, or a sales credit",
  "note — for a human to review. You can never approve, post, or finalise anything. A human",
  "bookkeeper approves every draft.",
  ...
  "",
  "Counterparties: match before create. If the document shows a NEW NAME with the SAME",
  "registration number as an existing counterparty, that is a rename of the same legal entity —",
  "propose the existing id and say so (the firm records the old name as an alias); never create",
  "a duplicate.",
  "",
...
// chatTurn.v4.impl.ts) fetches sha256 / resolution / books_version / op_key
...
export const draftJournalEntryInputSchema = z.object({
  ...
  counterparty: z
    ...
    .describe(
      "The counterparty (the supplier on a supplier_bill; the CUSTOMER on a sales entry): an " +
        "existing id, or a proposed new one (match-before-create; a new name with the same " +
        "registration number is a rename, not a new counterparty).",
    ),
  evidence: z
    .array(
      z.object({
 ...
 */
export function toTypedParts_v4(content: readonly AiContentPart[]): ClaraPart[] {
  const out: ClaraPart[] = [];
