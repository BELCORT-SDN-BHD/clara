// @frozen
//
// Frozen witness-facts STEPS (F-A1 design §3.1). Infrastructure is process-injected via
// globalThis so pool / storage / model-adapter tuning stays OUTSIDE the immutable workflow
// closure — the exact AB-16 precedent documentIngest_v1, invoiceFacts_v1 and statementFacts_v1
// all use. Step IO carries only the task id and small receipts: bytes, credentials, the raw
// region text and the provider payload never cross a WDK boundary.
//
...
// instead of buying two more model calls. Folding the two reads into one processing step — the
// statementFacts shape — would make every persist retry re-egress and re-bill; that is the one
// place this class deliberately diverges from its sibling.
//
...
} from "./witnessFacts.v1.behavior.mjs";
...
function services(): WitnessFactsServices {
  const value = (globalThis as unknown as { __claraWitnessFactsServices?: WitnessFactsServices }).__claraWitnessFactsServices;
  if (!value) throw new Error("witness-facts services not injected (globalThis.__claraWitnessFactsServices)");
  return value;
}
 ...
 *  and never loops. */
export async function claimWitnessFactsTaskStep(
  taskId: string,
...
export async function witnessTextReadStep(taskId: string, doc: ClaimDoc | null): Promise<WitnessTextRead> {
  "use step";
...
export async function witnessVisionReadStep(taskId: string, doc: ClaimDoc | null): Promise<WitnessVisionRead> {
  "use step";
...
export async function persistWitnessFactsStep(
  taskId: string,
