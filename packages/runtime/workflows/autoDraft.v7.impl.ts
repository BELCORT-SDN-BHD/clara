...
//
// THIS FILE (impl) — an UNMODIFIED version-rename of v6: every step body
...
// only import paths and the SYSTEM_PROMPT_AUTODRAFT_V6 -> _V7 identifier moved. F9's
// change lives in the prompt/tools/errors modules this file imports.

import { streamText, stepCountIs } from "ai";
import { getWritable, getWorkflowMetadata } from "workflow";
import type { ModelMessage } from "ai";
import {
  SYSTEM_PROMPT_AUTODRAFT_V7,
  DRAFT_TOOL,
  toAutoDraftOutcome,
...
import { pools, resolveModel, type ToolCtx } from "./autoDraft.v7.infra.js";
import { buildAutoDraftTools } from "./autoDraft.v7.tools.js";

export { SYSTEM_PROMPT_AUTODRAFT_V7 };
 ...
 *  assigned to the thrown Error here is therefore INVISIBLE to autoDraft.v7.ts's top-level
 *  catch, which only ever sees the reconstructed FatalError, not this original object.
 *  refusalFromCaughtError (autoDraft.v7.ts) parses this exact prefix back out. */
...
export async function consumeAutoDraftModelResult(
  ...
  try {
    ...
    if (streamError != null) {
      ...
      // what autoDraft.v7.ts eventually sees.
...
export async function runAutoDraftModelStep(
  ...
  const result = streamText({
    ...
    system: SYSTEM_PROMPT_AUTODRAFT_V7,
    messages,
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    tools: tools as any,
