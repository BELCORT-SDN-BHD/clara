// @frozen
//
// FROZEN — the autoDraft_v5 tool set + the draft_journal_entry write-floor wrapper
...
//
// v5 vs v4 (ledger #46, owner ruling 2026-07-29 — the SST-zero precedent): the ONLY change
// in this file is the DRAFT_TOOL's own `.describe()` string below, which echoes
// autoDraft.v5.prompt.ts's purchase-leg SHAPE rule to the model at the tool-call surface —
// "a STATED tax" narrows to "a STATED NONZERO tax", matching the owner's ruling that a
// stated-but-zero tax figure (the shape the client's own four approved EZSEC entries — all
// printing "SST Amt @ 6%: 0.00" — were actually posted under) takes the two-leg shape, not
// the three-leg sst_purchase_cost visibility split. Everything else — the tool set, the
// read/write shapes, and the draft-wrapper's server-side logic — is an unmodified
// version-rename of v4.
//
// v4 vs v3 (ledger #44/GitHub #42): this file was an unmodified version-rename of v3 — the
// tool set, the read/write shapes, and the draft-wrapper were unaffected by that wave's fix.
//
// v3 vs v2 (Wave B, AMB-1/FORK-6): the draft-wrapper's internal server-side re-fetch
// (runDraftJournalEntry, below) KEEPS purpose "coding" and does NOT set the GUC — it only
// needs books_version, so it stays wiki-dark by design (AMB-1). The get_context_pack TOOL
// exposed to the model is the one call site here that feeds MODEL context: its `purpose`
// input is pinned to the literal enum "wiki_coding" (z.literal — a model-supplied free
// string can no longer reach the DB as a different purpose, per FORK-6(b)), and its execute
// sets the txn-local `clara.pack_consumer = 'v25'` GUC immediately before the pack fetch, on
// the SAME client inside the SAME readScoped transaction.
...
import { refusalFromDbError } from "./autoDraft.v5.errors.js";
import { readScoped, writeScoped, safeRead, type PgExec, type ToolCtx } from "./autoDraft.v5.infra.js";
