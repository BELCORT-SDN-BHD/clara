...
export type ClientOnboardingInput = { clientId: string; planId: string; startedBy: string };
export type ClientOnboardingOutcome = { planId: string; clientId: string; outcome: "interview_complete" | "cancelled" | "expired" | "plan_gone" | "superseded_by_existing_run"; answered: number };
...
export async function clientOnboarding_v1(input: ClientOnboardingInput): Promise<ClientOnboardingOutcome> {
  ...
  async function persistSegment(seg: (typeof CLIENT_SEGMENTS)[number], res0: Extract<Awaited<ReturnType<typeof askAndConfirmSegment>>, { outcome: "answered" }>) {
    let res = res0;
    let attempt = 0;
    for (;;) {
      ...
      const reEcho = await askAndConfirmSegment(seg, ask, prior);
