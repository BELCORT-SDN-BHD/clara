...
// body is IMMUTABLE once any run can be in flight. chatTurn_v9 is a NEW frozen closure
// beside the byte-untouched chatTurn_v8/v7/v6/v5/v4/v3/v1 (the registry repoints chatTurn:
// here; prior exports stay until zero non-terminal runs of those versions remain). Do
// NOT edit this file or its import closure (chatTurn.v9.impl / .tools / .infra /
// .prompt / .errors) once deployed.
//
// chatTurn_v9 (§7-A THE UNATTENDED SALES DRAFTER; wave-7a-contract.md §3 PR-CHATTURN,
// skeleton §2f, ADR-063) ships ONE behavioural change, confined entirely to
// chatTurn.v9.prompt.ts's system prompt text (7A-R4's floor-purity fix reinforced at
// the prompt layer — full rationale in that file's own header): everything in THIS
// file is an unmodified version-rename of v8.
//
// chatTurn_v8 (carried) shipped THREE functional changes, everything else an
// unmodified version-rename of v7:
//
//   #46a (the diagnostic twin) — THIS file's ONLY v8 change: the catch block now
...
//   fault actually said. chatTurn.v9.impl.ts's consumeChatTurnModelResult (a
...
//   is NOT safe to write there. Every caught error — tagged or not — settles
//   error_code = 'model_error', the SAME value v7 always wrote; error_code does
//   NOT differentiate a stream error from any other model-segment failure. The
//   diagnostic value #46a actually adds lives entirely in the tagged MESSAGE, not
//   this column.
//
...
//   chatTurn.v8.tools.ts) — full rationale in that file's own header (carried
//   verbatim into chatTurn.v9.prompt.ts, plus this wave's one new sentence).
//
// chatTurn_v9 carries everything v8 carried: the durable chat turn WITH the Slice-6
// coding floor, the sales direction, the generic journal_entry lane, the SST
// registration-watch + purchase-visibility framing, and wiki awareness (every
// get_context_pack fetch that feeds MODEL context runs purpose "wiki_coding" with
// the txn-local clara.pack_consumer = 'v25' GUC). The schema + steps are otherwise
// byte-identical to v8: like v1 (load snapshot -> load history + pack -> loop model
// segments, checkpoint + stream each, park on clarify, settle idempotently) PLUS
// in-turn attachment perception, the firm-scoped read tools, the draft_journal_entry
// write tool, and — the C-19 terminal invariant — a coding-intent turn NEVER settles
// silently: it ends with a je_review card, a typed clarify, or a typed refusal.
// je_review/refusal parts are deduped as they accumulate so a WDK replay never
// double-appends a card.
...
import {
  ...
  settleStep,
  closeStreamStep,
} from "./chatTurn.v9.impl.js";
import { CLARIFY_FRAMING, type ClaraPart } from "./chatTurn.v9.prompt.js";
import { codingIncompleteRefusal } from "./chatTurn.v9.errors.js";
 ...
 *  consumeChatTurnModelResult's own tagged THROWN message (chatTurn.v9.impl.ts),
...
export async function chatTurn_v9(input: { taskId: string }): Promise<{ taskId: string; outcome: string; segments: number }> {
  "use workflow";
  ...
  try {
    ...
    const task = await loadTaskStepV9(taskId);
    const ctx = await loadContextStepV9(task.sessionId, task.clientId, task.firmId, task.createdBy);
    ...
    for (; segment < MAX_SEGMENTS; segment++) {
      const seg = await runModelSegmentStepV9(taskId, task.model, task.clientId, task.firmId, task.createdBy, messages, systemExtra);
