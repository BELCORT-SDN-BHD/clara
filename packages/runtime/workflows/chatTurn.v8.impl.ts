// @frozen
//
// FROZEN — the durable "use step" bodies of chatTurn_v8 (owner-approved closing
// batch, 2026-07-29). Every step's DB effects are workflow semantics and therefore
// frozen (ship a change as a new _vN). Infrastructure + the tool set live in the
// sibling frozen modules chatTurn.v8.infra.ts / chatTurn.v8.tools.ts (imported
// RELATIVELY so the freeze-lint follows + hash-locks them); this file holds only
// the steps + the attachment-aware transcript folder. Per-attempt credentials are
// minted inside the step that uses them and never cross a step boundary (§4.1).
//
// v8 vs v7 (ledger #46a, the diagnostic twin — owner-approved 2026-07-29): the ONLY
// change in THIS file is runModelSegmentStepV8's stream consumption. v7 (like
// autoDraft's own v3, before ledger #44) piped every `fullStream` part to the
// workflow's writable UNINSPECTED, then blindly awaited result.content/.totalUsage
// with no try/catch — ai@7's own internal stream-text processor sometimes rejects
// those promises with the SPECIFIC upstream error, but can ALSO fall back to a bare
// NoOutputGeneratedError("No output generated. Check the stream for errors.") with
// NO cause at all, discarding whatever `{type:'error'}` part the SAME fullStream
// loop had already seen pass through — exactly the swallow ledger #44 found and
// fixed in autoDraft.v4.impl.ts's consumeAutoDraftModelResult (full diagnosis in
// that file's own header; the live finding was task
// 65f293ba-ab00-4136-95da-8ffbd42e14fd / run wrun_01KYP1D0V61F9XS1Y4Z66GX10Z).
// consumeChatTurnModelResult below is a DUPLICATED, cross-referenced port of that
// EXACT pattern — not a shared import: a versioned workflow must never couple its
// shape to another workflow FAMILY's frozen file (chatTurn and autoDraft are
// independent closures), so this is chatTurn's own local copy, tagged with its own
// CHATTURN_MODEL_ERROR_TAG to keep the two families' diagnostic channels distinct.
// Every OTHER step body in this file is an unmodified version-rename of v7.
...
import {
  ...
  type AiContentPart,
  type JeReviewPart,
} from "./chatTurn.v8.prompt.js";
import { pools, resolveModel, readScoped } from "./chatTurn.v8.infra.js";
import { buildToolsV8 } from "./chatTurn.v8.tools.js";

export { SYSTEM_PROMPT_V8 };
...
function messageFromParts_v8(role: string, parts: ClaraPart[]): ModelMessage | null {
  const text = parts
    .map((p) => {
...
export async function loadTaskStepV8(
  taskId: string,
  ...
  return pools().withRuntime(async (c) => {
    ...
    if (r.rowCount === 0) throw new Error(`chatTurn_v8: task ${taskId} not found`);
    const row = r.rows[0]!;
    return {
...
export async function loadContextStepV8(
  sessionId: string,
  ...
  const history = await pools().withRuntime(async (c) => {
    ...
    return r.rows
      .map((row) => messageFromParts_v8(String(row.role), (row.parts ?? []) as ClaraPart[]))
 ...
 *  chatTurn.v8.ts's top-level catch, which only ever sees the reconstructed
 *  FatalError, not this original object. errorCodeFromCaughtError (chatTurn.v8.ts)
...
export async function consumeChatTurnModelResult(
  ...
  try {
    ...
    if (streamError != null) {
      ...
      // load-bearing for what chatTurn.v8.ts eventually sees.
...
export async function runModelSegmentStepV8(
  taskId: string,
  model: string,
  clientId: string | null,
  firmId: string,
  ...
  const tools = buildToolsV8({ firmId, clientId, createdBy, taskId });
  const result = streamText({
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    model: resolveModel(model) as any,
    system: systemExtra ? `${SYSTEM_PROMPT_V8}\n\n${systemExtra}` : SYSTEM_PROMPT_V8,
    messages,
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    tools: tools as any,
