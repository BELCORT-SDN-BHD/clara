...
// once any run can be in flight. clientOnboarding_v2 is the F1/F2 successor to
// clientOnboarding_v1; v1 stays byte-identical, exported and registered so every parked run —
// and the ≥48h parks are the whole point of this class — finishes on the semantics it started
// with. The registry points NEW admissions here. Ship the next behavioural change as _v3.
//
// clientOnboarding_v2 — the durable client identity interview. Identical to v1 in every
// durability property: the plan/run binding written as the FIRST plan item ('interview_run',
// closing the /start double-submit race), one update_onboarding_plan CAS write per confirmed
// answer with the F6 re-echo instead of a last-writer-wins overwrite, the stable per-attempt
// op_keys, and P19 (validated, echo-confirmed, and only then persisted).
//
// THE DIFFERENCE is the question set and the driver: CLIENT_SEGMENTS_V2 through the v2 driver.
// For a client this matters more than for a firm — a Malaysian firm's client base is mostly
// sole proprietorships, partnerships and LLPs, i.e. precisely the entity shapes for which v1's
// registration validator refused a legitimate ROB number outright and its framework question
...
export type ClientOnboardingV2Input = { clientId: string; planId: string; startedBy: string };
export type ClientOnboardingV2Outcome = { planId: string; clientId: string; outcome: "interview_complete" | "cancelled" | "expired" | "plan_gone" | "superseded_by_existing_run"; answered: number };

export async function clientOnboarding_v2(input: ClientOnboardingV2Input): Promise<ClientOnboardingV2Outcome> {
  "use workflow";
