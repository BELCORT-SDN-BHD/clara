...
// body is IMMUTABLE once any run can be in flight. chatTurn_v6 is a NEW frozen closure
// beside the byte-untouched chatTurn_v5/v4/v3/v1 (the registry repoints `chatTurn:` here;
// prior exports stay until zero non-terminal runs of those versions remain). Do NOT
// edit this file or its import closure (chatTurn.v6.impl / .tools / .infra / .prompt /
// .errors) once deployed.
//
// chatTurn_v6 — the durable chat turn WITH the Slice-6 coding floor (contract §3),
// the Wave-A2 sales direction (v4), the generic journal_entry lane (v5), AND the
// Wave-A2.1 compliance-watch + purchase-visibility PROMPT framing (v6 delta, prompt-
// only — the schema + steps are byte-identical to v5): surface the SST registration
// watch unprompted with its basis label (never a legal determination, never ×8%), and
// guide the purchase 3-leg visibility split (expense-net + a tied sst_purchase_cost
// leg + AP gross) when a supplier bill's facts state a tax amount. Like v1 (load
// snapshot -> load history + pack -> loop model segments, checkpoint + stream each,
// park on clarify, settle idempotently) PLUS: in-turn attachment perception, the
// firm-scoped read tools, the draft_journal_entry write tool (v6: coding_kind
// supplier_bill | sales_invoice | sales_credit_note | journal_entry->NULL), and —
// the C-19 terminal invariant — a coding-intent turn NEVER settles silently: it ends
// with a je_review card, a typed clarify, or a typed refusal. je_review/refusal
...
import {
  ...
  settleStep,
  closeStreamStep,
} from "./chatTurn.v6.impl.js";
import { CLARIFY_FRAMING, type ClaraPart } from "./chatTurn.v6.prompt.js";
import { codingIncompleteRefusal } from "./chatTurn.v6.errors.js";
...
export async function chatTurn_v6(input: { taskId: string }): Promise<{ taskId: string; outcome: string; segments: number }> {
  "use workflow";
  ...
  try {
    ...
    const task = await loadTaskStepV6(taskId);
    const ctx = await loadContextStepV6(task.sessionId, task.clientId, task.firmId, task.createdBy);
    ...
    for (; segment < MAX_SEGMENTS; segment++) {
      const seg = await runModelSegmentStepV6(taskId, task.model, task.clientId, task.firmId, task.createdBy, messages, systemExtra);
