// @frozen
//
// FROZEN — the durable "use step" bodies of autoDraft_v1 (Wave A auto-draft sweep). Every
...
// autoDraft.v1.infra.ts / autoDraft.v1.tools.ts (imported RELATIVELY so the freeze-lint
...
import {
  SYSTEM_PROMPT_AUTODRAFT,
  DRAFT_TOOL,
  toAutoDraftOutcome,
  type AutoDraftOutcome,
  type AiContentPart,
  type JeReviewPart,
} from "./autoDraft.v1.prompt.js";
import { pools, resolveModel, type ToolCtx } from "./autoDraft.v1.infra.js";
import { buildAutoDraftTools } from "./autoDraft.v1.tools.js";

export { SYSTEM_PROMPT_AUTODRAFT };
...
export async function runAutoDraftModelStep(
  ...
  const result = streamText({
    ...
    system: SYSTEM_PROMPT_AUTODRAFT,
    messages,
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    tools: tools as any,
