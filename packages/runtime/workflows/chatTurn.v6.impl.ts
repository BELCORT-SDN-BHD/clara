// @frozen
//
// FROZEN — the durable "use step" bodies of chatTurn_v6 (Slice-6 coding floor).
...
// chatTurn.v6.infra.ts / chatTurn.v6.tools.ts (imported RELATIVELY so the freeze-lint
...
import { pools, resolveModel, readScoped } from "./chatTurn.v6.infra.js";
import { buildToolsV6 } from "./chatTurn.v6.tools.js";

export { SYSTEM_PROMPT_V6 };
...
function messageFromParts_v6(role: string, parts: ClaraPart[]): ModelMessage | null {
  const text = parts
    .map((p) => {
...
export async function loadTaskStepV6(
  taskId: string,
  ...
  return pools().withRuntime(async (c) => {
    ...
    if (r.rowCount === 0) throw new Error(`chatTurn_v6: task ${taskId} not found`);
    const row = r.rows[0]!;
    return {
...
export async function loadContextStepV6(
  sessionId: string,
  ...
  const history = await pools().withRuntime(async (c) => {
    ...
    return r.rows
      .map((row) => messageFromParts_v6(String(row.role), (row.parts ?? []) as ClaraPart[]))
...
export async function runModelSegmentStepV6(
  taskId: string,
  model: string,
  clientId: string | null,
  firmId: string,
  ...
  const tools = buildToolsV6({ firmId, clientId, createdBy, taskId });
  const result = streamText({
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    model: resolveModel(model) as any,
    system: systemExtra ? `${SYSTEM_PROMPT_V6}\n\n${systemExtra}` : SYSTEM_PROMPT_V6,
    messages,
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    tools: tools as any,
