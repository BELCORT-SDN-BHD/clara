...
// --- Fail-closed local gate (#1018: shared with every other standalone World e2e driver). Same
// composed shape as `work-journal-e2e.mjs`'s, deliberately: this file spawns the same server,
// against the same throwaway databases, and a gate that admitted one more name here would be a
// second, looser answer to one question. That file's gate moved to admit the riders wave's
// per-lane `clara_l<NN>` (#980); this one moves with it, for the same reason and in the same
// shape, so the two stay one answer. Still loopback-only, still a parsed-DSN equality check
// against the PG env, still fail-closed on anything else.
assertLocalDbGate({
  label: "periodic-adjustment-e2e",
