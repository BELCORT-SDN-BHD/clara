 ...
 * This file swaps engines several times over (first, narrator, poster, faulty, respawned) against
 * the SAME database, so every respawn after the first can borrow a predecessor's still-fresh beat.
 *
...
async function main() {
  ...
  try {
    ...
    try {
      await waitReady(45000, narrator);
