...
test("f-a2.pr2.registry: chatTurn and autoDraft are repointed, and v8/v9/v12/v13/v14/v15 stay EXPORTED so no parked run is stranded", () => {
  ...
  assert.equal(registry.workflows.chatTurn.name, "chatTurn_v18", "#623 repointed chatTurn past FS-7's v17, P6-1's v16 and the earlier v15/v14/v13 pins");
  ...
  assert.equal(registry.autoDraft_v10, registry.workflows.autoDraft, "the newly pinned body stays directly addressable by workflow id");
  ...
  assert.equal(registry.chatTurn_v18, registry.workflows.chatTurn);
