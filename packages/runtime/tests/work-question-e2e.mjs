...
function spawnServe(port, extra = {}) {
  ...
  const state = { exited: false, exitInfo: null, banner: null, control: [] };
  child.on("exit", (code, signal) => {
    state.exited = true;
    state.exitInfo = { code, signal };
  });
  child.stdout.setEncoding("utf8");
  let pending = ""; // partial trailing line between chunks — see the [control] note below
  child.stdout.on("data", (d) => {
    ...
    // them in order) AND retained on `state` (so a failure message can quote them).
    //
    // Buffered to LINE boundaries: a stdout chunk can split a line in half, and a diagnostic that
    // silently drops the one line a future failure needs is worse than none.
    pending += d;
    const lines = pending.split("\n");
    pending = lines.pop() ?? "";
    for (const line of lines) {
      if (!line.includes("[control]") && !line.includes("CONTROL listening")) continue;
