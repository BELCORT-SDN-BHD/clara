...
| `CLARA_STORAGE_PROBE_CACHE_MS` | Background interval after the immediate boot probe; default `60000`. Accepted timer range is `1000..2147483647` ms; finite values outside it are clamped (non-finite values use the default) with one warning naming the variable and effective bound, so Node never converts an overflow to a 1 ms hot loop. The configured pair must also satisfy `3 * CLARA_STORAGE_PROBE_TIMEOUT_MS <= CLARA_STORAGE_PROBE_CACHE_MS`; an invalid pair falls back to the default timeout and, when needed to preserve the relation, the default cache. `/ready` never waits for this interval or performs probe I/O itself. **BREAK-GLASS:** set this very high (within the accepted range) and redeploy so only the eager boot probe runs in the operational window and the storage gate cannot re-fire; this helps only if that eager boot probe succeeds, deliberately suspends ongoing storage assurance, and must be reverted after recovery. Storage down at boot remains 503 by design. |
| `CLARA_STORAGE_PROBE_TIMEOUT_MS` | Per-cycle write+readback deadline; default `3000`. Accepted timer range is `1000..2147483647` ms, subject to `3 * timeout <= cache`; finite out-of-range values are clamped and non-finite values use the default, with a warning. A relation violation falls back to `3000` ms (and the cache default too if required to keep the relation true). The deadline aborts both storage calls and their fetches. Refresh ownership then waits up to another 2x deadline for settlement and scratch cleanup; an unabortable filesystem hang cannot own the slot forever. Re-evaluate the Fly readiness grace below before raising this value above `3000`. |
