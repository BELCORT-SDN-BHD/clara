// Pre-integration gate for #975's fix round (migration 0281). NOT a test file: preload it for an
// estate sweep run against a chain that predates it, so the sweep greens with a LOUD skip instead
// of hard-failing the four cells that assert post-0281 behaviour — a per-year arrears figure in
...
// file, so the variable stays unset and a chain missing 0281 FAILS LOUDLY. Final acceptance is
