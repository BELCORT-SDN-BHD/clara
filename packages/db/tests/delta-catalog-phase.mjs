...
await t.test("writer APIs are authenticated-only security definers with pinned search_path", async () => {
  ...
  for (const [name] of DELTA_ENTRYPOINTS.filter(([candidate]) => candidate !== "verify_evaluator_freeze")) {
    ...
    // The NON-INHERITING login shells are named explicitly because a group probe cannot answer
    // for them; F-A6's group + its login join the list for the same reason (the to_regrole guard
    // below keeps the cell green on a frontier where they do not exist yet).
    for (const [role, expected] of [[ROLES.authenticated, true], [ROLES.agentRo, false], [ROLES.runtime, false], [ROLES.wakeInteractive, false], [ROLES.wakeProactive, false], ["clara_agent_read_login", false], ["clara_wake_write_login", false], ["clara_freeform_ro", false], ["clara_freeform_login", false]]) {
