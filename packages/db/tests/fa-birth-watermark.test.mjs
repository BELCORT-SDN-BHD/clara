...
import {
  ...
  faRegisterTie, mon, dayIn, COST, ACCUM, EXPENSE, BANK,
...
test("p972.law 0247 applied: the birth body carries the BELT's own watermark expression exactly once, 0216's watermark-free join is gone, and t_je_fa_acquisition_birth is still the deferred insert-or-update constraint trigger whose re-fire is the mechanism", async (t) => {
  ...
  // The SAME words as the belt. Two instruments that disagree about which entries are in scope is
  // the defect class 0247 closes; identical phrasing is what keeps them from drifting apart.
  const belt = (await rootQuery(
    "select p.prosrc as src from pg_proc p where p.oid = 'clara._tf_fa_movement_belt()'::regprocedure",
  )).rows[0].src;
  assert.equal(occurrences(belt, WATERMARK_EXPR), 1,
    "clara._tf_fa_movement_belt still phrases the watermark in exactly these words (0041 §S2.6)");
