...
//   clara.list_accrual_adjustments(p_client, p_from, p_to, p_side?, p_cursor?, p_limit?) --
//       #1075 (0334) added p_side, server-side, default null (every side); #1152 (0365) added
//       p_cursor/p_limit, both default null (a null pair reproduces the pre-0365 door exactly,
//       no default page size) -> {client_id, from, to, side, accruals[], next_cursor}
...
export async function listAccrualAdjustments(sub, {
  ...
  // every OTHER caller of this fixture (the base #652 battery gated on 0222 alone, #942's own
  // revenue-side battery gated on 0304, and accrual-list-side-filter.test.mjs's own `side`-only
