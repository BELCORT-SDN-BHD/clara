// Pre-integration gate for the statutory-deadlines DDL train. NOT a test file: it is preloaded
// by the package test script (node --test --import ./tests/statutory-deadlines-preintegration-
// gate.mjs tests/) so the package-wide sweep, which may run against a database that predates
// this migration, greens with a LOUD skip instead of a silent one-armed skip with no CI net
// (f-a7-alpha-preintegration-gate.mjs's own idiom, mirrored byte-for-byte).
//
...
//
// A FOCUSED invocation (node --test tests/statutory-deadlines-ddl.test.mjs) does not preload
