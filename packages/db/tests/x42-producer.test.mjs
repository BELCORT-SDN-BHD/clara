 ...
 *  packages/db/tests/split-lists/test-list-d-b2.txt's own `#!cells-floor:` count) still runs
 *  this file against the D-b2/D-b3 frontiers (0041_asm + the chain through 0044/0045), where
 *  the producer is exactly as designed and this whole battery must stay green. So this is a
...
before(async () => {
  ...
  if (await producerRetired()) {
    live = false;
    noteLane("F-A3 PR-3 retires clara.accept_bank_rule_suggestion whole (Annex I) — the x42 PRODUCER battery is dormant on this frontier; db-slice-frontiers still proves it green at D-b2/D-b3 (0041_asm..0045)");
    return;
  }
