...
// AFTER the F-A1 PR-1 llm_witness arm this file's own RESTORE_FA1 layer reverses, inside the
// SAME outer if/elsif/end-if 0038 itself later reverses wholesale -- which is why this layer
// must be undone before 0038's). PURELY ADDITIVE: the reversal pair's "to" side is the empty
// string, since removing the whole new arm is exactly what "before gamma" means here -- no
// existing text is edited, only inserted after. The pair is MACHINE-EXTRACTED (read live off
// this lane's own rig via pg_proc.prosrc, never retyped by eye), matching the discipline every
// earlier layer here states.
const RESTORE_F_A7_GAMMA = {
"router": [
[
...
function applyRestoreF_A7_Gamma(member, src) {
  for (const [frm, to] of RESTORE_F_A7_GAMMA[member]) {
    // DORMANCY vs DRIFT, the same discipline every layer here uses. On a chain where gamma has
    // NOT been applied the router carries no classify arm and there is nothing to reverse --
    // that is a pre-gamma database, not a broken one, so the layer is a no-op.
    const n = src.split(frm).length - 1;
    if (n === 0) continue;
    if (n !== 1) {
      throw new Error(`F-A7 gamma restore(${member}): the classify-arm pair appears ${n} times -- the live body drifted from the ratified F-A7 gamma shape`);
    }
    src = src.replace(frm, to);
  }
  return src;
