...
import {
  ...
  UNKNOWN_VERDICT, canonical, WIKI_PURPOSE,
...
test("[0020 §9.2 — RPR's exact production shape]: a client with ONLY a live legacy null-purpose row keeps invoice-facts AUTHORIZED and wiki synthesis HELD (verdict unknown)", async () => {
  ...
  // [Wave-F Track A, F-A7 gamma] narrowed to the WIKI purpose specifically, not "any typed
  ...
  // The invariant this cell actually proves — a legacy grant never touches typed WIKI state —
  // still holds and is what is asserted below.
  assert.equal(await rootQuery(
    "select count(*)::int n from clara.client_egress_purpose_consents where client_id=$1 and purpose=$2",
    [client, WIKI_PURPOSE]
  ).then((r) => r.rows[0].n), 0, "…and no typed WIKI row was created for it by anything");
