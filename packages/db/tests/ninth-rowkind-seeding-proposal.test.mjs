...
test("MED-3 (Codex cross-model review, fec6ab5b): ALL NINE row_kinds, produced through real state, EACH asserted present at its FULL 30-key shape — a missing kind is a FAIL, not a silently-skipped observation", async () => {
  ...
  for (const row of rows) {
    ...
    assert.deepEqual(
      ...
      `row_kind='${row.row_kind}' (id=${row.id}) carries a DIFFERENT key set than the pinned shape (now 31 keys, #974/0260 added authority_id) — a key was added, dropped or renamed (got ${JSON.stringify([...Object.keys(row)].sort())})`,
    );
