...
test("§3.6 begin_chat_turn same-turn_key replay returns the ORIGINAL task (one task, ONE user message); a different key on a live turn → CLR13", async (t) => {
  ...
  // Terminal-settle frees the slot; the next turn admits.
  await settleChatTurn({ task: taskIdOf(first), tokens: 5, outcome: "completed" });
...
test("§6 budget (ratified D-A): used ≥ budget refuses CLR14+copy; at budget−1 admission serializes (proven) and may admit ≤ cap; usage lands at settle then the boundary closes", async (t) => {
  ...
  // Materialize the daily row organically, then pin usage as the operator
  // (root) — the discovered counter column is reported for the record.
  const setup = taskIdOf(await beginChatTurn({ session: sessions[0], author: owner, turnKey: opk("b0") }));
  await settleChatTurn({ task: setup, tokens: 1000, outcome: "completed" });
  ...
  // (c) Each admitted turn's settle LANDS its usage exactly once, and the
  ...
  for (const [i, side] of [out.winner, out.loser].entries()) {
    if (!side?.ok) continue;
    const tk = 50 + i * 20;
    await settleChatTurn({ task: taskIdOf(side.receipt), tokens: tk, outcome: "completed" });
    settled += tk;
  }
  assert.ok(settled > 0, "at least the winner settles");
  assert.equal(await usedNow(), before + settled, "every admitted turn's settle landed exactly its tokens on the daily counter");
...
test("§3.6 settle_chat_turn replay is a stored-outcome no-op: task_usage single row, daily total unchanged, assistant parts not rewritten", async (t) => {
  if (unready(t)) return;
  ...
  const first = await settleChatTurn({ task, tokens: 500, outcome: "completed", parts: [{ type: "text", text: "first answer" }] });
  ...
  // Replay with DIFFERENT tokens/parts — must be a no-op returning the stored outcome.
  ...
  assert.deepEqual(stripReplayFlag(replay), stripReplayFlag(first), "the terminal replay returns the STORED outcome");
