...
test("wc.1 cancelling a QUEUED Work terminalises both halves, cancels its pending question and posts nothing", async (t) => {
  ...
  const q = await park(w.task_id).catch(() => null); // a queued task cannot park — tolerated
  ...
  if (q !== null) {
    ...
    assert.ok(rows.every((r) => r.status !== "pending"), "wc.1 no question is left pending");
  }
});
...
test("wc.9 a CANCELLED Work refuses work_cancelled; a SETTLED one refuses work_settled", async (t) => {
  ...
  assert.equal(await committedReceiptCount(refused.client) >= 0, true);
});
...
test("wc.13 the mirror never re-opens a Work that already settled on its receipt", async (t) => {
  ...
  await post(w);
  await cancelAccountingWork({ work: w.work_id, author: BOB() });
  assert.equal((await workRow(w.work_id)).status, "completed", "wc.13 precondition: completed");
  const out = await cancelAgentTask(BOB(), { task: w.task_id });
  assert.equal(out.status, "completed", "wc.13 the task was already terminal");
  assert.equal((await workRow(w.work_id)).status, "completed", "wc.13 the Work is untouched");
});
...
test("wc.15 a committed receipt still outranks the cancel translation", async (t) => {
  if (await gateCancel(t)) return;
  const w = await running();
  await post(w);
  ...
  assert.equal((await taskRow(w.task_id)).status, "cancel_requested", "wc.15 precondition");
  ...
  assert.equal(out.status, "completed", "wc.15 the BOOKS win over both the run and the cancel");
  assert.equal(out.overridden_by_receipt, true);
  assert.equal(out.translated_by_cancel, false, "wc.15 …so no cancel translation happened");
  const row = await workRow(w.work_id);
  assert.equal(row.status, "completed");
  assert.equal(row.error, null, "wc.15 a completed Work carries no error");
});
...
test("wc.22 a REVOKED actor reads nothing of the firm's Work, receipts or journal; a live colleague reads them", async (t) => {
  ...
  assert.equal(revoked.questions, 0, "wc.22 …and ZERO questions");
...
test("wc.23 a DEMOTION to viewer leaves the firm-visible reads intact — measured, not changed", async (t) => {
  ...
  const r = await humanQuery(demoted,
    `select (select count(*)::int from clara.accounting_work where id=$1) as work,
            (select count(*)::int from clara.operation_receipts where work_id=$1) as receipts`,
    [w.work_id]);
  noteLane(`wc.23 measured: a demoted viewer reads work=${r.rows[0].work} receipts=${r.rows[0].receipts}`);
