// Pre-integration gate for 裁-41's client_identifiers UNIQUE. NOT a test file: it is preloaded by
// the package test script (node --test --import ./tests/client-identifiers-unique-preintegration-
// gate.mjs tests/) so the package-wide sweep, which may run against a database that predates this
// migration, greens with a LOUD skip instead of a silent one-armed skip with no CI net
// (statutory-deadlines-preintegration-gate.mjs's own idiom, mirrored).
//
...
//
// A FOCUSED invocation (node --test tests/client-identifiers-unique.test.mjs) does not preload
...
process.env.CLARA_ALLOW_MISSING_CLIENT_IDENTIFIERS_UNIQUE = "1";
