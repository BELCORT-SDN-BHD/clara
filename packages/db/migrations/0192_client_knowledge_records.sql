...
--     register, and a legacy fact whose key now carries a live knowledge record is shadowed by
--     it. The five legacy catalog keys are carried into `knowledge_keys` BY VALUE (a SELECT from
...
--     the third belt: a key the catalog marks `authority_bearing`, and every `policy` key,
...
--     admin+. A client row SHADOWS a firm row of the same key on read, so an established client
--     exception survives a later firm default.
--
...
--   · no consumer is built for the events this file registers.
-- =====================================================================================
...
insert into clara.knowledge_keys
    (knowledge_key, kind, value_shape, validated_against, allowed_values, description,
     scope_default, authority_bearing)
...
-- B.1 — THE APPLICABILITY DIGEST, twice over and deliberately so.
--   · the COLUMN is GENERATED ... STORED from an inline, provably-immutable expression
--     (`convert_to` is STABLE, so the estate's usual sha256(convert_to(...)) shape is REFUSED by
--     a generation expression -- measured on PG17; `jsonb_pretty(x)::bytea` is the immutable
--     route, and jsonb_pretty prints a jsonb object with its keys already normalised, so
--     {"a":1,"b":2} and {"b":2,"a":1} hash the same).
--   · the FUNCTION below is the SAME expression for the doors' own lookups. It is deliberately
--     NOT used inside the generated column: a user function in a generated expression is a
--     pg_dump ordering dependency this estate's DR round-trip has no reason to take on. §I's tail
--     asserts the two agree.
create function clara._knowledge_applies_when_digest(p jsonb) returns text
  language sql immutable parallel safe as
  $$ select encode(sha256(jsonb_pretty(coalesce(p, '{}'::jsonb))::bytea), 'hex') $$;
...
create table clara.knowledge_records (
  ...
  applies_when_digest  text        generated always as
                         (encode(sha256(jsonb_pretty(applies_when)::bytea), 'hex')) stored,
  effective_from       date,
  effective_to         date,
...
create function clara._knowledge_insert_revision(
  ...
  language plpgsql security definer set search_path = clara, pg_temp as $$
declare v_new uuid; v_version bigint; v_event text; v_digest text;
begin
  ...
  perform clara._audit(p_firm, p_asserted_by, null, null, p_fn, null,
    jsonb_build_object('record_id', coalesce(p_record_id, v_new), 'revision_id', v_new,
      ...
      'recorded_via', p_recorded_via, 'knowledge_version', v_version));
  ...
  return jsonb_build_object(
    ...
    'knowledge_version', v_version, 'superseded_id', p_prior_id);
end $$;
  ...
  if found then
    if p_on_conflict = 'skip' then
      return jsonb_build_object('status', 'skipped', 'record_id', prior.record_id,
        ...
        'knowledge_key', p_knowledge_key, 'knowledge_version', prior.knowledge_version);
...
create function clara._knowledge_floor(p_knowledge_key text, p_scope_kind text) returns text
  ...
  select case when p_scope_kind = 'firm' then 'admin'
              when k.kind = 'policy' or k.authority_bearing then 'admin'
              else 'bookkeeper' end
    from clara.knowledge_keys k where k.knowledge_key = p_knowledge_key;
$$;
...
create function clara._knowledge_row_json(r clara.knowledge_records) returns jsonb
  ...
  select jsonb_build_object(
    ...
    'recorded_at', r.recorded_at, 'knowledge_version', r.knowledge_version,
    ...
    'superseded_at', r.superseded_at, 'state', r.state, 'editable', true);
$$;
...
--   · the FIRM defaults this client does not override -- a live client row of the same key
--     SHADOWS them (#603 Q22: a firm default preserves established client exceptions);
--   · the LEGACY clara.client_facts live rows, as `source_kind='legacy_client_fact'` and
--     `editable=false`, shadowed the same way by a knowledge record of the same key.
...
begin
  ...
  select coalesce(jsonb_agg(j order by knowledge_key, recorded_at desc), '[]'::jsonb), max(ver)
    into v_rows, v_version
    from (
      select clara._knowledge_row_json(r)
          || jsonb_build_object('asserted_by_name', u.display_name,
             ...
             r.knowledge_version as ver, r.knowledge_key as knowledge_key,
  ...
  -- it is READ here so C13 shows one register, shadowed by a knowledge record of the same key.
  select v_rows || coalesce(jsonb_agg(j order by fact_key), '[]'::jsonb)
    ...
    from (
      select jsonb_build_object(
          ...
          'key_description', kk.description) as j,
       ...
       where cf.client_id = p_client and cf.firm_id = c.firm and cf.superseded_at is null
         and not exists (select 1 from clara.knowledge_records o
                          where o.firm_id = c.firm and o.scope_kind = 'client'
                            and o.client_id = p_client and o.state = 'live'
                            and o.knowledge_key = cf.fact_key)
    ) l;
  return jsonb_build_object('client_id', p_client, 'knowledge_version', coalesce(v_version, 0),
    'records', v_rows);
end $read$;
...
begin
  ...
  select coalesce(jsonb_agg(j order by knowledge_key), '[]'::jsonb), max(ver)
    into v_rows, v_version
    from (
      select clara._knowledge_row_json(r) as j, r.knowledge_version as ver,
  ...
  return jsonb_build_object('status', 'ok', 'client_id', p_client, 'firm_id', v_firm,
    'purpose', p_purpose, 'knowledge_version', coalesce(v_version, 0), 'records', v_rows);
end $read$;
...
create function clara.promote_plan_answers_to_knowledge(
    p_plan uuid, p_op_key text, p_promote_firm_scope boolean default false) returns jsonb
  language plpgsql security definer set search_path = clara, pg_temp
  set plan_cache_mode = force_custom_plan as $door$
declare
...
begin
  ...
  -- THE TWO LANES. A human arrives with a JWT and meets the admin floor; clara_runtime arrives
  -- without one and is admitted by its EXECUTE grant alone (PostgREST never reaches this role).
  v_actor := clara.jwt_sub();
  if v_actor is not null then
    c := clara._human_ctx(clara.role_rank('admin'));
    if c.firm <> p.firm_id then
      raise exception 'onboarding plan not found' using errcode = 'CLR11';
    end if;
  ...
  return clara._finish_op(p.firm_id, 'promote_plan_answers_to_knowledge', p_op_key,
    jsonb_build_object('plan_id', p_plan, 'scope_kind', v_scope, 'client_id', v_client,
      ...
      'knowledge_version', coalesce(v_version, 0)));
end $door$;
revoke all on function clara.promote_plan_answers_to_knowledge(uuid, text, boolean) from public;
...
grant execute on function clara.promote_plan_answers_to_knowledge(uuid, text, boolean)
...
begin
  ...
  -- 5 · THE DIGEST IS ONE EXPRESSION, stated twice: the generated column and the doors' helper.
  select pg_get_expr(adbin, adrelid) into v_s from pg_attrdef d
    join pg_attribute a on a.attrelid=d.adrelid and a.attnum=d.adnum
   where d.adrelid='clara.knowledge_records'::regclass and a.attname='applies_when_digest';
  if v_s is null or position('sha256' in v_s)=0 or position('jsonb_pretty' in v_s)=0 then
    raise exception '#644 tail: applies_when_digest is not the generated sha256(jsonb_pretty(...)) expression (got %)', v_s
      using errcode='CLR10';
  end if;
  ...
  if not (has_function_privilege('clara_authenticated',
            'clara.promote_plan_answers_to_knowledge(uuid, text, boolean)'::regprocedure, 'EXECUTE')
      and has_function_privilege('clara_runtime',
            'clara.promote_plan_answers_to_knowledge(uuid, text, boolean)'::regprocedure, 'EXECUTE')
      and not has_function_privilege('clara_agent_ro',
            'clara.promote_plan_answers_to_knowledge(uuid, text, boolean)'::regprocedure, 'EXECUTE')) then
