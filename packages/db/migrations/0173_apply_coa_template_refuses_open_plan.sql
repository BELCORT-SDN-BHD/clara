...
--
-- THE REFUSAL IS CLEARABLE BY A HUMAN, so an abandoned interview strands nothing. A client whose
-- NEWEST plan is open takes no chart until it leaves 'open', and two granted doors do that:
-- clara.commit_client_onboarding is the ordinary exit the ruling waits for, and
-- clara.cancel_client_onboarding (0017_wave_b.sql:2843 -- admin rank, EXECUTE to
-- clara_authenticated at :5143, reason + op_key required) withdraws an abandoned one. NOTED, not
-- introduced here: cancelling ARCHIVES the client (:2866) and this door gates on firm not status.
--
