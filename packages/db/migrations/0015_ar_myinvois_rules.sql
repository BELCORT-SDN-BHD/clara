...
declare v_key text; v_facts text[]:=array[
    ...
    'invoice.type_code','invoice.total_excl_tax','invoice.tax_total',
...
declare
  v_ext uuid; v_sup_reg text; v_cust_reg text; v_sales boolean:=false; v_cust boolean:=false;
begin
  ...
  if v_cust_reg is not null and exists(select 1 from clara.client_identifiers ci
      where ci.client_id=p_client and ci.kind in ('tin','ssm')
        and ci.value_normalized=v_cust_reg) then
    v_cust:=true;
  end if;
...
begin
  ...
  -- S6: additive sales / SST fact fields (all null for AP purchase docs).
  select min(monetary_cents) into v_net from clara.document_regions
    where extraction_id = v_ext and field_path = 'invoice.total_excl_tax';
  select min(monetary_cents) into v_tax from clara.document_regions
    where extraction_id = v_ext and field_path = 'invoice.tax_total';
  select nullif(btrim(min(text_content)),'') into v_type from clara.document_regions
  ...
  if v_engine like 'clara-%' then
    -- STRUCTURED Tier-A (§3.5): schema-parsed facts corroborate WITHOUT geometry
    -- when the arithmetic tie holds. Only the invoice type (01) corroborates the
    -- invoice-total equation; CN/DN (02/03) + self-billed carry their own ties in
    -- the shape floor but are corroboration-INELIGIBLE for the total here.
    v_ok := v_total_count = 1 and v_total is not null and v_total > 0
      ...
      and coalesce(v_type,'01') = '01'
      and (v_net is null or v_tax is null or (v_net + v_tax) = v_total);
  else
...
create function clara._assert_sales_invoice_shape(p_entry uuid) returns void
  language plpgsql security definer set search_path = clara, pg_temp as $$
declare
  e record; v_state jsonb; v_gross bigint; v_net bigint; v_tax bigint;
  v_recv bigint; v_rev bigint; v_sst bigint; v_sst_acct text;
begin
  select * into e from clara.journal_entries where id = p_entry;
  if not found then return; end if;
  if e.coding_kind not in ('sales_invoice','sales_credit_note') or e.reversal_of is not null then
    return;
  end if;
  ...
  -- SST account presence (only demanded when tax facts are actually present).
  ...
  -- Tie on stated facts FIRST: net + tax must EXACTLY equal gross (v1 declares no
  -- rounding fact, so a <=5-sen mismatch surfaces here rather than silently drifting
  -- into the rounding leg — adversarial #9).
  if v_net is not null and v_tax is not null and (v_net + v_tax) <> v_gross then
    raise exception 'sales tax breakdown does not tie to the gross total'
      using errcode='CLR21',detail='{"reason":"tax_tie_failed"}';
  end if;
  if e.coding_kind = 'sales_credit_note' then
...
begin
  ...
  -- whole-entry constraint (adversarial #5/#6): EVERY non-control, non-rounding leg
  -- must hit the rule's account on the direction-correct side (purchase=>debit,
  -- sales=>credit); and at least one such leg must exist.
  if exists(select 1 from clara.journal_lines l
      join clara.coa_accounts a on a.client_id=l.client_id and a.account_code=l.account_code
      where l.entry_id=p_entry
        and a.account_class is distinct from 'payable'
        and a.account_class is distinct from 'receivable'
        and a.special_acc_type is distinct from 'rounding'
        and (l.account_code<>r.account_code
          or (v_direction='purchase' and not (l.debit_cents>0))
          or (v_direction='sales' and not (l.credit_cents>0))))
     or not exists(select 1 from clara.journal_lines l
      join clara.coa_accounts a on a.client_id=l.client_id and a.account_code=l.account_code
      where l.entry_id=p_entry and l.account_code=r.account_code
        and a.account_class is distinct from 'payable'
        and a.account_class is distinct from 'receivable'
        and ((v_direction='purchase' and l.debit_cents>0)
          or (v_direction='sales' and l.credit_cents>0))) then
    insert into clara.rule_post_skips(firm_id,client_id,entry_id,rule_id,reason)
  ...
  -- Drive the SAME approve core with the rule identity. Benign race codes become
  -- skips (review M2); any other exception propagates honestly.
  begin
...
begin
  ...
  for elem in select value from jsonb_array_elements(p_fields) loop
    ...
    v_cents:=case when v_path in ('invoice.total','invoice.amount_due','invoice.deposit',
                  'invoice.total_excl_tax','invoice.tax_total')
