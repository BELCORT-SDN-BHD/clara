...
begin
  ...
  -- ADDED AT INTEGRATION (riders sweep wave), with the recut of clara._tenancy_plan_core's own
  -- authority block: that step now DELEGATES to lane L1's shared wall instead of carrying 0300's
  -- copy of it, so the predicate is a prerequisite of this file rather than an option. A
  -- prerequisite is a prestate refusal, not a body that would fail at its first call. 0330 is the
  -- lower number and ships in the same merge, so on every chain this file can reach it is there.
  if to_regprocedure('clara._assert_plan_authority(text,jsonb,uuid,uuid)') is null then
    raise exception '#1137 prestate: clara._assert_plan_authority is absent -- clara._tenancy_plan_core delegates to it since the integration recut, so 0330_plan_authority_wall_predicate must apply first'
      using errcode='CLR10';
  end if;
...
begin
  ...
  -- THE AUTHORITY SHAPE, and after the riders sweep wave there is exactly ONE of it.
  --
  -- RECUT AT INTEGRATION (riders sweep wave, L1's 0330 against this file). As written on the lane
  -- these 42 lines were 0300's authority block pasted whole, under the comment "THE AUTHORITY
  -- SHAPE, verbatim from clara.create_accounting_plan". That claim was true when this lane was cut
  -- from the cut head, and false the moment lane L1 merged: #1051 (0330) extracts the wall out of
  -- clara.create_accounting_plan and clara._obo_plan_core into the shared
  -- clara._assert_plan_authority, #1080 (0331) points clara._accrual_plan_core at it, and #1050
  -- (0338) does the same for every kind but its own. This paste would have been the FOURTH
  -- hand-written copy of a wall the wave exists to fold, and it would have been the only one left
  -- standing.
  --
  -- IT IS NOT A DISCLOSED RESIDUAL, because #1051's cells MEASURE it: p1051.wall.one_definition
  -- refuses any clara body that both calls the predicate and keeps a copy of the wall's own
  -- sentence, and p1080.wall.one_spelling asks the same question across the whole schema. Riders
  -- wave 4's fourth lesson, learned on this body's own sibling clara._obo_plan_core: a disclosed
  -- residual is only safe while nothing measures it.
  --
  -- NOTHING THIS STEP ADMITS OR REFUSES MOVES. The predicate admits the same three reference
  -- kinds, raises the same sentences with the same SQLSTATE and the same `detail` payloads, and
  -- resolves through the same #977 resolver this block called directly -- which is what 0330's own
  -- tail proves, on all eight refusal axes. The parity this file's p1137.obo.refusals_match and
  -- p1137.obo.plan_step_parity measure becomes exact BY CONSTRUCTION rather than by transcription:
  -- the tenancy step and clara.create_accounting_plan now reach the same body.
