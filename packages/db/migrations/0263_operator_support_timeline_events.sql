...
begin
  ...
  select count(*)::int into v_n from clara.event_types
   where name = 'admission.capacity_set';
  if (v_n > 0) is distinct from v_applied_cap then
    raise exception '#843 prestate: HALF-APPLIED -- admission.capacity_set registered: %, clara.set_admission_capacity appends it: %. The catalog and the door must move together; this file refuses to build on the gap.', (v_n > 0), v_applied_cap
      using errcode='CLR10';
  end if;

  raise notice '#843 prestate: clean -- clara.set_admission_capacity exists exactly once at (integer,text,text), carries 0186 §C''s owner floor, operator fragment, reservation pair, audit call and advisory lock, and its posture (clara_fn_owner, SECURITY DEFINER, search_path + plan_cache_mode pinned, PUBLIC-revoked, EXECUTE to clara_authenticated only) is the one create-or-replace is about to preserve; clara.reject_firm_registration still carries the audit-then-append pair this recut copies; an active taxonomy version exists. Re-apply over this file''s own effects (redo #957): %.', v_applied_cap;
end
$w843_pre$;
...
select a.version, t.name, 'context_update', null
 ...
 cross join (values ('admission.capacity_set')) as t(name)
...
begin
  ...
  raise notice '#843 tail: OK -- clara.set_admission_capacity exists exactly once at (integer,text,text), calls clara._audit exactly once and clara._append_event exactly once, the append names admission.capacity_set under the operator firm with a null client and an EMPTY payload, it sits strictly between _reserve_op and _finish_op so a replayed op_key writes no second line, the door''s posture (clara_fn_owner, SECURITY DEFINER, search_path=clara, pg_temp + plan_cache_mode=force_custom_plan, PUBLIC-revoked, EXECUTE to clara_authenticated only) is byte-identical to what §0.6 measured before the recut, and admission.capacity_set is registered exactly once as a firm-level type routed context_update at the active taxonomy version with no catalog row left unrouted.';
end
$w843_tail$;
