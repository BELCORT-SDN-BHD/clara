...
begin
  ...
  raise notice 'identity tail: OK -- % alias row(s) all carry a lane (default legacy_unknown, NOT NULL, honesty trigger installed); the origin CHECK admits five values; eight provenance constraints and three firm-congruent source FKs landed; clara.counterparty_identity_revisions FORCEs RLS with the counterparty_merges owner+human policy pair, SELECT to clara_authenticated alone, zero DML, and its append-only + no-truncate pair enabled; three lane-agnostic revision triggers cover the two declared-residue writers; clara.add_counterparty_alias is EXACTLY ONE body at the widened ten-argument signature with clara_authenticated EXECUTE restored; the three reads are SECURITY DEFINER FUNCTIONS pinning search_path and plan_cache_mode with no new masked view minted; counterparty_aliases_visible projects twelve columns behind its kept security_barrier and firm predicate while the base table gained NOTHING; the four event types route context_update at taxonomy version % with the coverage anti-join empty; no machine role and no PUBLIC grant reaches anything here; no wake_fn_allowlist row and no clara.unmerge_counterparties exists.',
