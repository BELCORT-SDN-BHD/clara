...
-- THREE FILES for the repo's 500-line cap, applied in ALPHABETICAL order (identity_helper ->
...
begin
  ...
  v_currency := upper(regexp_replace(coalesce(v_reg->'invoice.currency'->>'raw_text',''), '[^A-Za-z]', '', 'g'));
  ...
  v_type := v_reg->'invoice.type_code'->>'text';
  ...
  if jsonb_typeof(v_tans) <> 'object' or jsonb_typeof(v_vans) <> 'object' then
    ...
    foreach v_f in array v_belt loop
      ...
      -- envelope answer vs the server-verified citations: they must not contradict.
      if (v_ts = 'value' and v_n <> 1) or (v_ts = 'not_printed' and v_n <> 0) then
        v_answers_ok := false; continue;
      end if;
      -- C2 (design §1, §3.4): every witnessed amount binds server-side to a layout region with
      -- real geometry. A missing/failed citation persists the fact geometry-less (the
      -- permissive-writer/strict-reader split) and this is the strict half.
      if v_ts = 'value'
      ...
      elsif v_f = 'invoice.currency' then
        v_ttxt := upper(regexp_replace(coalesce(v_reg->v_f->>'raw_text',''), '[^A-Za-z]', '', 'g'));
        v_vtxt := upper(regexp_replace(coalesce(v_va->>'raw',''), '[^A-Za-z]', '', 'g'));
        if nullif(v_ttxt,'') is null or nullif(v_vtxt,'') is null or v_ttxt <> v_vtxt then
          v_agree_ok := false;
        end if;
      else -- invoice.type_code
        v_ttxt := v_reg->v_f->>'text'; v_vtxt := nullif(btrim(coalesce(v_va->>'raw','')),'');
        if v_ttxt is null or v_vtxt is null or v_ttxt <> v_vtxt then v_agree_ok := false; end if;
      end if;
    end loop;
  end if;
  v_vcurrency := upper(regexp_replace(coalesce(v_vans->'invoice.currency'->>'raw',''), '[^A-Za-z]', '', 'g'));
  ...
  v_ok := v_answers_ok and v_agree_ok and v_c2_ok
    ...
    -- MYR ASYMMETRY, BOTH CHANNELS (design §3.3; invoice-currency-reader.mjs:280-306's
    -- confirm-or-refuse law). Absence yields '' <> 'MYR' and never corroborates — 0023:306's
    -- behaviour preserved, and behaviourally OPPOSITE to the STATEMENT posture's absence->MYR
    -- (WC-R5); design §3.7 records that the divergence must not be silently unified.
    and v_currency = 'MYR' and v_vcurrency = 'MYR'
  ...
  -- 5. IDENTITY — delegated whole to the frozen closure leaf clara._witness_identity_v1 (its own
  ...
  v_ident := clara._witness_identity_v1(p_document, v_text_id, v_contest);
  ...
  v_out := jsonb_build_object(
    ...
    -- has one; the extra disjunct only ever ADDS refusals.
    'explicit_non_myr', (nullif(v_currency,'') is not null and v_currency <> 'MYR')
                     or (nullif(v_vcurrency,'') is not null and v_vcurrency <> 'MYR'));
...
-- clara._witness_identity_v1 (the identity verdict leaf; the design's three-member list predates
...
begin
  select sha256(convert_to(string_agg(
    ...
    from (values (0,'clara.evaluate_witness_fact_state_v1(uuid,uuid,uuid)'),
                 ...
                 (3,'clara._witness_identity_v1(uuid,uuid,boolean)')) m(o,s);
...
begin
  ...
  foreach v_src in array array['_fact_hash','_normalize_invoice_cents','_witness_identity_v1'] loop
  ...
  -- and friends are relations, not functions, so the only names this can find are calls.
  select string_agg(distinct m[1], ', ') into v_src from regexp_matches(
      (select p.prosrc from pg_proc p where p.oid='clara.evaluate_witness_fact_state_v1(uuid,uuid,uuid)'::regprocedure),
      'clara\.(_[a-z0-9_]+)\s*\(', 'g') as m;
  if coalesce(v_src,'') <> '_fact_hash, _normalize_invoice_cents, _witness_identity_v1' then
    raise exception 'F-A1 part1 tail: the predicate calls clara functions [%] — the frozen closure covers exactly [_fact_hash, _normalize_invoice_cents, _witness_identity_v1]; a called leaf outside the closure is the half-freeze this discipline exists to prevent', coalesce(v_src,'(none)')
      using errcode='CLR10';
  end if;
