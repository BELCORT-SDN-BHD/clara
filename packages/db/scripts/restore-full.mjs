...
function printChecklist(log = console.log) {
  const lines = [
    ...
    "     workflow_drizzle.workflow_migrations == source BEFORE starting a worker, else",
    "     the engine silent-no-ops or collides on CREATE SCHEMA. Only then CLARA_START_WORLD=1.",
